...
| **The CCRP impact plane was the RELEASE POINT's ground, not the target's** — found here, REPAIRED since: the briefed fix carries the terrain the mission sampled at it, and `FBNavSystem::SetSteerpoint` no longer accepts a loose probe. Over a 0 m plane the two are equal by construction, so the error was exactly zero and invisible | 1.8: plane 511.9 m vs target 834.4 m = **+322.43 m** error → 230.49 m off, target survived, computer reporting `aimMissM` 65.3 m. After: plane error **0.00 m**, `aimErrM` **22.14 m**, `s8cmd` DESTROYED, `s8vam1` SUCCESS. Over the eight sorties the summed delivery error falls **629.9 → 247.1 m** and the spread **10–230 → 15–46 m**; 1.3 loses a span kill that the −34.8 m plane error had granted (see its header) |
