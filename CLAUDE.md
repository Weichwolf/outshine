...
their textured path has to be right. But the budget is laid out for five things, and they are the
answer to "what actually looks stunning":

| | in the tree, measured |
|---|---|
| **high geometry, generators RECURSIVE** | the cluster DAG is built and proven — and subjects do not go through it yet |
| **many lights, and shadows** | the cap is **16**; the shadow atlas stands, cascades are open |
| **realistic atmosphere** | **19** catalogue rows: transmittance, multi-scatter, radiance, sky-view, irradiance |
| **parameterised materials** | `MetalRoughBrdf` with Fresnel and both lobes |
| **reflections and mirroring** | **nothing. No SSR, no probe, no cubemap, no stage** |

A texture is somebody else's photograph of a surface; light and composition are the engine's own
and are what a viewer reads. The frame budget is finite, so every millisecond has a place it did
NOT go, and this table is where it goes.

**The procedural half is nearly true already, measured rather than hoped.** The driven world holds
**4 surfaces and 4 images** over 10 draws and 11 504 triangles. Sponza's 25 images and
VirtualCity's 167 are VENDOR corpus, not this engine's content.

Two things follow and they change priorities rather than adding work. Lighting and reflections
outrank the indirect draw, because that is where the value is. And SDL_GPU's want of bindless —
which caps a pass at one draw per material class — bounds how well this engine draws OTHER
PEOPLE'S textured assets, not its own world, where the class count is four.
