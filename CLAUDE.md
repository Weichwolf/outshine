...
**WHERE THE BUDGET GOES: GEOMETRY, LIGHT AND SHADOW — and as close to TEXTURE-FREE as the work
allows.** A texture is somebody else's photograph of a surface. Light and composition are the
engine's own, they are what a viewer actually reads, and they are worth more. This is a bound on
the middle clause above rather than a taste: the frame budget is finite, so every millisecond has
a place it did NOT go.

**It is nearly true already, measured rather than hoped.** The driven world holds **4 surfaces and
4 images** over 10 draws and 11 504 triangles. Sponza's 25 images and VirtualCity's 167 are VENDOR
corpus, not this engine's content — the corpus is the oracle and its textured path must draw
correctly, but it is not what the engine is FOR. outshine's own world is procedural and its
materials are parameters.

Two things follow and they change priorities rather than adding work. Lighting outranks the
indirect draw, because light is where the value is. And SDL_GPU's want of bindless — which caps a
pass at one draw per material class — bounds how well this engine draws OTHER PEOPLE'S textured
assets, not its own world, where the class count is four.
