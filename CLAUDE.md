...
| `CorridorLay` | `[[nodiscard]] bool LayCorridor(const Path::Route &route, const GroundQuery &ground,` (CorridorLay.h:59) -- same shape, same question. The product it lays no longer holds bands that must agree by convention: `std::vector<Station> Fine;` (:32) is one extent and `At(double alongM)` (:46) one clamped index, so an unlaid corridor refuses at the tick's entry instead of returning a default per read (board:1820) |
