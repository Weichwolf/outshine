...
| `Engine` | the door LAYS the drive now -- `bool Engine::Assemble() {` (Engine.cpp:135) reaches `if (!Sim::AssembleDrive(S_->Scene, S_->Stood, S_->Vehicles, S_->Drives, declared.Ground,` (:165), `bool Engine::Advance() {` (:455) ticks it and `bool Engine::Drove(void) const { return S_->Drove; }` (:186) answers. Amber for two reasons the screenshot found: a route that fails makes the WHOLE assembly fail, so the product draws nothing at all (board:1870), and the door recovers its own refusal by grepping printed prose -- `if (line.rfind("REFUSED", 0) == 0 && Why.empty()) { Why = line; }` (:35). Views, Input, Volumes, Tables and Sounds are still accepted and never advanced (board:1862) |
