...
| the mesh the DAG starts from must not be coarser than the tolerance the DAG declares | a level-0 cluster carries `SelfErr` = 0, which asserts the drawn surface IS the DEM. It is not: the tile mesh is a decimation and `err` is its own miss. `kEdgeTau / kGrid` (`world/World.cpp`) is that miss in pixels and it must stay within one factor of `SseTauPx` — **3 px against 1 px** since 2026-08-08, four before that on the same day, twelve before that. MEASURED while it was twelve: `FB_TAU` 1 → 0.25 moves the silhouette by under 0.05 px, i.e. the cut had nothing left to select ([`../../world/terrain.md`](../../world/terrain.md) §2.2) |
| the tile's geometry costs ONE vertex per posting | the drawn mesh is a regular grid and the DAG welds it anyway, so the buffer is indexed and every level of the ladder shares that one array. Image-neutral by construction and measured so: **0 of 921 600 pixels** ([`../lod.md`](../lod.md) `## State`) |
