...
| 2.15 | **THE GCI SCAN-ELEVATION ENTRY IS A WORLD-FRAME ANGLE POSTED INTO A BODY-FRAME ANTENNA COMMAND.** `FBMig29Pilot` computes `elDeg = atan2(g.AltM − ownAlt, g.RangeM)` and posts it to `FBCommandTarget::RadarSlewEl`; this file's own uncued search law posts the identical geometry **minus `st.pitch`**, with the reason written beside it (*"der eigene Nick macht daraus ein Kommando statt einer Konstante: das Muster ist an die Nase geschraubt"*). On a level CAP the two agree inside `kInterceptElDeadDeg` and no committed mission has ever exposed it. [MESS, `o5-02-scramble`] a pair climbing at **25.0 m/s** at **+5.86…+6.13°** of pitch: the raid sits at **−3.1…−4.2° in the body frame** at 57 → 25 km, well inside the N019's ±6.0° RAD bar, and the antenna is commanded to **+2.83°** where the pilot's own law would command **−3.03°** — the bar's lower edge lands at −3.17° and the raid stays 0.5–1.0° outside it for the whole closure. **Zero radar contacts in 700 s over a 726 m closest approach**, and deleting the controller does not recover it (measured: the uncued law then aims at the briefed steerpoint's altitude, +5.6…+7.6°). It is worth the aircraft's whole pitch angle to every interceptor that is not in level flight when its controller calls. **A SIBLING OF THE SAME KIND WAS FOUND AND FIXED IN `modules/air` on 2026-07-29 — in AZIMUTH:** `set brief_gci <bearing>` is a TRUE bearing and `FBAirModule` posted it straight into `RadarSlewAz`, which is BODY-referenced, so `set brief_gci 90` parked a MiG-23's ±30° antenna 90° off its own nose and the intercept machine — which commands elevation only — never recovered it. Zero contacts before, first contact at 27.37 nm after, on an unchanged mission file. **A briefed call is a pair of numbers in the CONTROLLER's frame and both of them have to be converted against the aircraft's own instruments**; the tree has now made that mistake twice, in the two different angles. **MEASURED ON THE OTHER SIDE OF ITS THRESHOLD BY O2, 2026-07-29, and the bite condition is now a formula rather than a case.** The error IS `st.pitch`, exactly; `FBAutopilot` holds a climb pitch of **5.36…5.89°** at 450 kt and at 350 kt alike (the climb rate is rate-limited, so speed does not move it), and the N019's RAD bar is ±6.0°. On O2's arena the raid sits at ~0° in the body frame and the antenna is commanded to **+8.303°** where the body-frame command would be **+2.90°** — and the contacts SURVIVE, first track at t = 30.9 in both the 450 kt and the 350 kt climb, because the shifted bar still reaches it. The margin is **0.11 to 0.64 degrees**. So the defect deletes an intercept iff `|pitch| + |target's body-frame elevation offset| > 6.0°`, which is why O5's scramble lost everything (its raid ALSO sat 3–4° low, and the two errors added) and O2's climb loses nothing. O2's control run reproduces O5's: deleting the controller from a climbing interceptor gives 0 contacts either way ([`campaigns/o2-pvo-intercept.md`](campaigns/o2-pvo-intercept.md) §State) | [`campaigns/o5-airfield-defence.md`](campaigns/o5-airfield-defence.md) §State |
