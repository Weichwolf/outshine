...
| 2.12 | **There is no path from the intercept into the merge, and the merge cannot be scored.** `FBPilot` has exactly ONE transition into `Phase::Bfm` and it is the briefed `set task bfm` at spawn: `InterceptCommands` aborts at `InterceptAbortRangeNm` (5 nm) and never transitions, so no BVR mission in this tree ever enters close combat. Two consequences, both measured in [`doctrine-evolution.md`](doctrine-evolution.md) `E2`: the BFM phase writes **no `eng_*` column** (every `FBEngagement` note sits in the intercept path), so the tournament fitness reads level C as `GATE` on both sides and is blind to everything a merge moves; and the merge itself is not a gunfight — **70 merge runs produced 6 gun bursts, 0 `gun HIT` lines and 0 `dmg_hits`**, while what actually decides those runs is 2.9's CFIT (9 of 11 opposing results `CRASH`). Until this closes, `pilot_energy_frac` — a gene that only acts in `BfmCommands` — has no arena it can be measured in | [`doctrine-evolution.md`](doctrine-evolution.md) E-14, E-15 |
