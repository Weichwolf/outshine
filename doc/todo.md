...
| **1.2 The bake cache holds what the GPU consumes, on disk** | PNG and JPEG were chosen so a generated raster could travel over HTTP to a browser; that wire is deleted in 1.5, and the encoder tuning (`stbi_write_force_png_filter = 0`, `compression_level = 1`) is the symptom of forcing a wire format into a cache role. The cache stores an **SDL_GPU texture format** — SDL3 declares 106, including `BC*` and `ASTC_*` in `UNORM`, `UNORM_SRGB` and `FLOAT` — so a read is an upload, not a decode plus a repack. **Per kind, not one answer:** style raster and photo are colour and want `SRGB`; terrarium DEM is packed height and must not be sRGB or lossy. **On disk**, surviving a restart, because block compression is the expensive half of a cold bake and memory-only would pay it every start. Decode stays — upstream sends PNG and JPEG and we do not control that — so SDL3_image is a decoder dependency; `stb_image_write`'s 1 724 lines leave with `stb_image`'s 7 988 |
