...
| | |
|---|---|
| **Working on** | The ledger stops lying — `heapKB` is `sbrk(0)`, so eviction can never appear in it |
| **Scope** | `doc/requirements.md`: **1373 features, 210 ticked, 1163 open** · `doc/bugs.md`: **40 defects** |
| **Last accepted** | Vegetation growth forms — eight architectures, crown width 2.080 → 0.993 (`40f4706`) |
...
1. **The ledger stops lying.** `heapKB` is `sbrk(0)` (`bugs.md`, world and streaming) — a break that
...
2. **The eye in every telemetry row**, plus per-pass admission counters. A run whose subject is motion
...
3. **The 500 m walk gate.** 500 m of travel raises `tilesTotal` and `poolHttpGets`; it fails in CI, not
   in a browser. **Done when** it is a declared run that goes red on today's binary.
4. **Nothing streams during play** — the defect the gate above catches. From t=31 s to t=77 s of the
   owner's session, `poolHttpGets` 310 flat and `tilesBuilt` 0. **Done when** the gate goes green.
5. **Eviction, before more streaming.** Fields grow monotonically and their unit of removal does not
...
6. **The silent-success class**, all three sites at once (`bugs.md`, first section). `RoofSurface::Cover`
