...
| **1.2a One global hash-keyed disk cache** | content-addressed, on disk, shared by **every producer** — providers and generators, not a tile cache with others beside it. It subsumes `cache.c` (upstream bodies by path), `bake.c` (rasters by kind + z/x/y + a hand-maintained `FB_OSM_STYLE_VER`) and the one that does not exist: **`TreePrototype::Grow` costs 1 473–1 548 ms for 31 declarations at every bring-up**, recomputed from nothing. **The trap:** hashing only the inputs serves last week's tree from this week's grower, silently. The key covers the declared inputs, the **producer's identity and version**, and the payload's format — and a producer whose code changed while its version did not is the one failure the cache cannot catch, which the header must say. A miss and a hit must differ only in timing, never in the answer |
