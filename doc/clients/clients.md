...
| **the WATCHED view** ([`../mods.md`](../mods.md) §1) | **built, browser only.** `C` (or `?view=chase` at boot) puts the camera 62 m behind, 13 m above and 18 m right of the unit this client already flies — the mission's FIRST `unit` block, which is the actor every f22 reading rule names as its verdict — aiming 150 m AHEAD of it. Roll is dropped and heading/pitch lag with a 0.45 s time constant, because a camera on a fighter's body axes rolls 300 °/s with it. The cockpit overlay is off in this view (it would project a flight-path marker on a boresight the camera does not have), so the scene takes all 720 lines and the per-frame pass count is the documented no-HUD variant | measured in headless Chromium/WebGPU over all eight `mods/f22` sorties: `render passcount passes=6 clouds=1 hud=0` against `passes=7 … hud=1` in the cockpit, 60 fps with no dropped frame in 2 min per sortie (`cpuprof rafMs` median 16.67, max 17.16; loop 0.9–2.2 ms of it) |
