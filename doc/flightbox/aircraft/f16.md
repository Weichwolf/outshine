...
**Delimitation:** `doc/f16/` documents the REAL jet from the manuals (DCS F-16C Viper Guide / ED EA
Guide). This file documents the CODE. Where a number comes from `doc/f16/`, the reference is cited —
...
measured) and changed the tick wiring — §2.1/§2.2/§2.6/§3.3 of the body below are due for a check
...
1. **No TD box, no locked-target symbol.** `doc/f16/hud-symbology.md` knows neither (the radar-adjacent
...
3. **Further empty/NoOp slots:** `FBPropulsionSystem` (engine logic above the raw FDM) and
   ...
   `FBGunSystem`).
4. **`WeaponSelect` is deliberately `NotImplemented`** (`doc/f16/controls-commands.md` §6.6) — the jet
...
8. **No gun bore depression** — no angle can be evidenced in `doc/f16/`, hence 0°/0°, written out.
...
`FBF16Pilot.*` (together with `systems/FBPilot.*`). Already visible in the working copy and NOT YET
...
conservative p10 produced 11 instead of 14 kills and 21 s less funnel time in the sweep). **§3.3 (BFM
...
| Marker | Meaning |
|---|---|
| `[DOC]` | evidenced in `doc/f16/` (reference cited) |
...
Wiring (once each, by the owner `units/FBSimUnit` or by the spawn path `app/FBMissionBoot.h`):
...
| Defensive (RWR → CMDS) | 10 Hz | salvo intervals are tenths of a second (`doc/f16/defence-rwr-cm.md` §2.2); slower would quantise a salvo. Order = data flow: the receiver writes, the dispenser reads in the same tick. |
...
| `WeaponSelect` | Stores | **always rejected, `NotImplemented`** — `doc/f16/controls-commands.md` §6.6: the jet has it, FlightBox does not. A silent success would be a lie the pilot then flies on |
...
| `AlowFt` | — (UFC) | range per `FBF16Ufc::AlowInRange`; **in addition**: with the CARA unpowered `Inhibited/EffectPrecondition` — the entry stands, but the warning can never fire, and the pilot learns that (`doc/f16/controls-commands.md` §6.4) |
...
| `brief_alow_ft`, `brief_bingo_lbs`, `brief_master_arm`, `brief_weapon`, `brief_chaff_s`, `brief_release_s` | see `doc/mission-format.md` | is NOT applied here but handed to the pilot, who enters it later over the command bus — in the latency class of the operation and rejectable |
| `pilot_*` | see `systems/FBPilotTuning` | passed through entirely: the parameter set belongs to the PILOT, not to this airframe, so there is no second, drifting copy of the key table here |
...
**What is NOT here:** the phase machine. `Run()` is generic (`systems/FBPilot`); this class overrides
...
| Hook | Value | Provenance |
|---|---|---|
| `ApproachSpeedKt` | 165 kt | `[MESS]` — the documentation names an AoA target (11°), not a CAS. Trimmed, gear down, ~40 % fuel, the model holds 11.0° AoA at **164.9 KCAS** (gym calibration sweep). The command is an open-loop placeholder for a real AoA loop — faithful to THE MODEL's trim curve instead of a copied real-world number |
...
`make -C sim test-corner` (`app/FBTestCornerSpeed.cpp`) runs entry speed against instantaneous turn
...
| Hook | Value | Provenance |
|---|---|---|
| `BfmCornerSpeedKt` / `BfmCornerG` | 380 / 5.6 | `[MESS]` — the peak of the table |
| `BfmMinSpeedKt` | 300 | `[MESS]` — there the rate is ~17 % below the peak |
...
0.02·nz), so full deflection at corner buys ~5.6 g instead of the 9 g structural limit, and the best
turn rate is ~16 °/s instead of the real jet's ~20+. The measured corner nevertheless lies WITHIN the
...
| `InterceptAbortRangeNm` | 5 | `[ABL]` below that the Rmin of a BVR weapon and the merge are the same problem, and `doc/f16/` calls the fight from there on a dogfight, not an intercept |
...
`sim/src/modules/f16/FBF16Fcr.{h,cpp}` — a derivation of `systems/FBRadarSystem`.
...
`doc/f16/hud-symbology.md` documents **no** TD box and **no** locked-target symbol; its radar-adjacent
...
`sim/src/modules/f16/FBF16Datalink.h` (header-only) — a derivation of `systems/FBDatalinkSystem`.
...
(= primary actor, `doc/mission-format.md`). A documented placeholder for a real lead assignment, not a
model of one.
...
`sim/src/modules/f16/FBF16Rwr.h` (header-only) — a derivation of `systems/FBRwrSystem`. Every point
...
`sim/src/modules/f16/FBF16Cmds.{h,cpp}` — a derivation of `systems/FBCountermeasureSystem`.
...
|---|---|---|
| **Release state** | position + full velocity vector from the platform block | A store leaves the pylon with the carrier's motion (the separation IC in `app/FBMissionBoot.h` says the same); the station offset is metres against a fall of kilometres, so the computer works from the CG — the SMS's pylon geometry is not fire-control knowledge |
...
need to know where the target is GOING. This box therefore holds its OWN `systems/FBBfmTrack` instance,
...
release path, point-mass/drag effect of the carriage) is `systems/FBStoresSystem`. Here stands
...
trigger command, burst stream) is `systems/FBGunSystem`. Here: WHICH gun this jet carries (`kM61A1`
...
propose→commit/reject cycle from `doc/f16/controls-commands.md` §1.2.
...
**Why separate from the font system:** `render/FBHudFont.h` + `FBHudStage`'s text path are the GENERIC,
...
Source situation: `[DOC]` `doc/f16/hud-symbology.md`, DCS F-16C Viper Guide Part 16 p.706 as the frame
