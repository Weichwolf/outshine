...
[`../sim/weapons-and-damage.md`](../sim/weapons-and-damage.md). A weapon on a pylon has no behaviour
at all, only mass, drag and a model name (`core/FBStore.h`); its behaviour **is** the FDM it becomes
when it is released.

**Neighbours:** `doc/f16/flight-model.md` §10 (what the pinned f16 model declares about carriage),
`sim/assets/MODEL-DELTAS.md` (the delta list and its gate), [`f16.md`](f16.md) (the carrier),
[`../sim/fdm.md`](../sim/fdm.md) (how carriage and damage reach JSBSim).
...
| 2 | No enemy store family exists at all (R-27/R-73/R-60M class) — see [`mig29.md`](mig29.md) and roadmap R6. |
...
a real delta — lives with the carrier: [`f16.md`](f16.md), Gaps, **awaiting owner decision**.

## Knowledge

| Fact | Source |
|---|---|
| Carriage is physics, not bookkeeping: an occupied station is a JSBSim **point mass**, the sum of drag areas a named `fb-stores` **external force** — mass, CG, inertia and added drag come from the engine | [`../sim/fdm.md`](../sim/fdm.md) |
| Both mechanisms are populated at runtime through the model's own APIs; no model XML is patched for them | [`../sim/fdm.md`](../sim/fdm.md) |
| Without any load, neither is ever created — a clean jet computes bit-identically to one that never heard of stores | [`../sim/fdm.md`](../sim/fdm.md) |
| A released store is structurally a full unit: own FDM instance, own module from the same registry, own telemetry file, the same two judges; its end is a detonation, not a crash (`UNIT_RESULT … IMPACT`) | [`../sim/units-and-missions.md`](../sim/units-and-missions.md) |
| Its initial condition comes from the carrier state (position + station offset, carrier attitude, carrier velocity at that station including ω × r) — no trim, no invented ejector impulse | [`../sim/units-and-missions.md`](../sim/units-and-missions.md) |
...
| F-16 station geometry as the model itself gives it (tank butt line ±65 in, half span 180 in, CG station longitudinally) — and why it is longitudinally collapsed | [`f16.md`](f16.md), [`../sim/weapons-and-damage.md`](../sim/weapons-and-damage.md) |
