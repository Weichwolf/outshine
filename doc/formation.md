...
| # | Thing | Known from |
|---|---|---|
| **F6** | **ANSWERED (2026-08-04, `E20`) — a flight requires a cooperative terminal, and that is now a DECLARED statement rather than an accident.** The finding stands unchanged: `FBFlightPicture::BuildMembers` adds SELF and returns unless `state.Datalink.H.Readable()`, `flt_mates` (max) is **3 on every F-16 and 0 on every MiG-29** on `sat-02`/`sat-07`, and `shape-*` leaves the MiG separations at the baseline's metre. `E19` left two branches open and `E20` measured both: **(a) THE ASYMMETRY IS SOURCED.** The MiG's Lazur-M is a ground COMMAND channel and not a shared flight picture ([`modules/f16/cockpit-displays.md`](modules/f16/cockpit-displays.md) D7, [`modules/mig29/datalink-gci.md`](modules/mig29/datalink-gci.md) §4/§5.2), and `FBMig29Module` says so at its member: *"the cooperative terminal this jet does not have at all — its GCI link is a VOICE channel the pilot types in, not a track picture."* Lending it one would be inventing a data link the aircraft never had. **(b) THE OWED ALTERNATIVE CANNOT BE BUILT FROM THIS TREE'S EYE, and the arithmetic is decisive rather than a preference.** A station is a POSITION relative to a NAMED lead; `FBVisualContact` withholds range STRUCTURALLY (*"there is no HasRange bit, because no path in the tree could ever set one"*) and carries no identity (*"two MiG-29s on opposite sides produce the identical string"*). And at station it does not even carry the TYPE: the MiG's beam-on presented dimension is **17.32 m** (`FBMig29DamageLayout`, 2 × 8.66), which at the 1 852 m combat spread subtends **0.536°** against a Johnson recognition rung of 4.0 × 0.2° = **0.8°** — reached only inside **1 240 m**, identification inside **776 m**, both at the look-UP reference contrast and worse looking down. **So G1 is an F-16-only doctrine BY THE AIRFRAME, not by an omission**, and a station-keeping channel that named a mate would be a seventh registry reader — the exact thing the perception boundary exists to forbid. What is still owed is only the bookkeeping: the arena and the fitness must not grade a side on a gene its airframe cannot carry ([`doctrine-evolution.md`](doctrine-evolution.md) `E-29`) | `E19`, `E20` |
