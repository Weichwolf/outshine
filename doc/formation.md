...
| # | Thing | Known from |
|---|---|---|
| **F6** | **A flight only exists to its own members through the DATALINK block, so an airframe without one has no flight and G1 has no call site on it.** [MESS, [`doctrine-evolution.md`](doctrine-evolution.md) `E19`] `FBFlightPicture::BuildMembers` adds SELF and then returns unless `state.Datalink.H.Readable()`; the MiG-29 module has no `datalink` key at all. Measured on `sat-07` and `sat-02`: `flt_mates` (max over the run) is **3 on every F-16 and 0 on every MiG-29**, although each MiG declares `flight ia 1…4` and reports its briefed `flt_pos` 1…4. With no mate there is no lead member, `FormationCommands` never runs and `FormationStation` is never called — `shape-tight`/`shape-wide`/`shape-stacked` leave the MiG separations at **1394 / 3537 / 4113 m from the lead, the baseline's numbers to the metre**. F5's three ratios are therefore an F-16-only doctrine today, and `flt_pos` on a MiG is a briefed number with no behaviour behind it. **Owed:** either a non-cooperative station-keeping source (visual formation needs no terminal) or a declared statement that a flight requires a datalink | `E19` |
