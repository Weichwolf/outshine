...
|---|---|---|
| `C9` | ~~the MiG-29 module cannot fly `set task attack`~~ — **CLOSED 2026-07-30.** Built as a **director**, not as a release cue: the aircraft picks the release moment, the pilot flies an instruction. The acceptance was the one measurement that could catch a shortcut — same geometry, same `fab500` on both so ballistics are not a variable, F-16 CCRP **34.02 m** against MiG-29 OPT **65.65 m**. A director that beat the computer would have been the F-16's procedure in disguise. Refusal is its own sourced case: the ranging act has to happen kilometres before release and the only rangefinder reaches 6 km, so a run-in that leaves no room is denied (`mig29-opt-refused.fbm`) | **nothing any more.** This file's own requirement is met |
| `C8` | ~~no FAB-class bomb~~ — the FAB rows landed with the air-to-ground round (`9682448`) and the director flies them; **the rocket pod is still absent** | rocket attack only |
| `C1` | ~~no SAM, no AAA~~ — built (`99897b5`) and the umbrella works in both directions; O5 measured a belt firing **east at its own aircraft**, which is exactly what a *friendly* umbrella has to survive being read as | nothing; but read O5's fratricide measurement before writing missions 5 and 6 |
