...
| `C8` | **Store catalogue is Mk-82 / AIM-120 / AIM-9 / R-73 / R-27R.** No HARM, no LGB, no Mk-84, no cluster, no rocket pod, no FAB-class bomb. **HOME 2026-07-28: [`../air-to-ground.md`](../air-to-ground.md) §§2–3** — eight rows, two new `FBSeekerKind` values (`AntiRadiation`, `SemiActiveLaser`), one new release check (the FAB envelope) and one magazine field group (the rocket pod). Specified, nothing built | **2** (W3/W4 SEAD, O3 stores) | 2 | there is no such thing as a suppression element in the tree. **The contract's own headline:** the anti-radiation seeker is the RWR, so it has a bearing and never a range — which makes the real AGM-88 memory mode unbuildable here and the crew's shutdown a real countermeasure, on a derived law rather than a setting |
| `C25` | **No air-to-ground radar function.** `FBRadarSystem` filters `FBUnitKind::Aircraft` ([`../sensors.md`](../sensors.md) gap 3), so no aircraft radar ever finds a ground object; and the fire control samples the elevation provider only at the briefed steerpoint. **Specified 2026-07-28 in [`../air-to-ground.md`](../air-to-ground.md) §4 as RANGING ONLY** — one slant range and one point where the commanded antenna line meets the terrain, never a contact, never a track, never a map. The `Aircraft` filter stays | 0 | **4** | the two radars are very unequal here (APG-68: GM/GMT/DBS/SEA/FTT; N019: **nothing**) and the file states the inequality as a **result**. Two consequences fall out for free: mapping RADIATES, so it wakes an `emcon hold` site, and every `--elev const` mission is byte-identical because a flat plane is a flat plane |
| `C26` | **Suppressed and destroyed are the same word.** `CombatEffective()` on a site is `Structure` alone, so a battery with a dead radar and dead rails still scores as effective — [`../modules/ground/module.md`](../modules/ground/module.md) G2. **Specified 2026-07-28 in [`../air-to-ground.md`](../air-to-ground.md) §5** as two halves: a **mechanism** (`set emcon react` — the crew goes off the air on its own damage, the one cue it can legally have) and a **verdict** (`objective suppress unit\|team [emitting <s>]`, deferred, roster price **one bool**). `CombatEffective` is expressly NOT widened | **2** (W3-04/05, W4-07) | 3 | the honest limit is named with it: the crew reacts to damage, i.e. AFTER the round arrives, so FlightBox's anti-radiation weapon is harder to defeat than the real one |
| `C27` | **The pilot cannot take a suppression shot.** The attack phase releases on a ballistic countdown; a bearing-only weapon needs a different cue. **Specified 2026-07-28 in [`../air-to-ground.md`](../air-to-ground.md) §7 as ONE gate** — `set attack_mode arm`, the RWR block read like any instrument, the shot moment still briefed by `set brief_release_s`. Deliberately bounded: no threat-driven turn, no defensive reaction, no re-attack | 0 | 2 | **the weapon is measurable before the pilot learns anything**, which is why this is a separate ID and a separate round |
