...
| **the environment is RECORDED by the run and READ by the replay, never guessed** | `campaign-summary.txt` carries `time`/`elev`/`dem`/`base`/`threads`; a tree without the record is refused rather than replayed against a default. A snapshot is one more such input and enters that record |
...
|---|---|---|
| `tools/bake_dem.py` (*"not a build target — run only on a change"*) | the mod's `data/<region>.bin` | tile fetching, resampling |
...
| the environment record (`time`/`elev`/`dem`/`base`/`threads`) | **built**; the DEM is recorded by path, not by content |
