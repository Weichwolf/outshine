...
- [ ] **A client with an input medium exists, and a declared interactive scene runs.** `Scene::Kind` already has an interactive arm and `b83285f` deleted the only target that could execute one: the deleted walk client refuses it (`scene_is_interactive`), so the arm is unreachable and `src/clients/Walker.{h,cpp}` — the stance integrator behind the verb — is compiled with no caller (`doc/bugs.md`). This is not the character controller below it and not a verb; it is the seam without which none of the verbs under it can ever be shown to work, and `CLAUDE.md` § *Setup* already asserts it exists (*"the interactive client is a test; so is the frame oracle"*)
