...
- [ ] No `malloc` outside `core/io/` — **eight** sites in the program, in `world/terrain/osmmesh_terrain.cpp` (5: `:49`, `:67`, `:130` `malloc`, `:234`, `:252` `calloc`), `world/terrain/terrain.cpp` (2: `:85`, `:136`) and `clients/SimHost.cpp:186`, measured by `sim/tools/hardening_ledger.py` (`allocOutsideCoreIo`). *The **seven** first written here counted `malloc` and not `calloc`, missing `osmmesh_terrain.cpp:234` and `:252`, and included `generators/gate/SameRegionSamePlacement.cpp:315` — a declared negative that is not part of any program and is outside the ledger's subject.* `world/ChunkMesh.h`'s four are gone, taken by `Heap::Take`
...
- [ ] A C-ABI status reaches a house type at the boundary, so the attribute applies to it — `world/terrain/`'s status functions all return `int`, which is neither `bool` nor an enumeration, so no gate and no attribute reaches them. All seven are now checked at every call site (`houseCAbiDiscarded: 0`, `sim/tools/hardening_ledger.py`); `osmmesh_geo_to_tile`'s two were the last, and `doc/bugs.md` carries what they cost while they were open. **A count is not the mechanism**: a new call site is checked because a tool counts and reports it, where `core/GroundSample.h` and `core/WaterDepth.h` already show the shape that makes the unchecked use unspellable. Owed: one house function per C-ABI status, returning a two-state answer whose payload is unreachable from the failing state, so the C ABI keeps its `int` and no caller in the tree ever sees one
- [ ] A standpoint is a type that cannot be constructed outside the Web Mercator band, so every entry point is refused by construction and none of them repeats the test — three exist today and they disagree: `clients/Scene.cpp:69-78` refuses at the declaration, `clients/Sim.h:77` `SetStance` validates nothing, and `clients/WorldMain.cpp:117` reads `argv` through `std::atof` into `Sim::At(double, double)` and answers the North Pole with the ground of 85.0511 N (`doc/bugs.md`). `Clients::Sim::Stance` is four defaulted doubles — `C.41`, `C.42`: a record whose validity depends on a field it does not require. The band is `world/terrain/geo.h`'s `osmmesh_mercator_lat_max_deg`, so the type belongs in `world/`, and `Sim::At` then takes it instead of two loose doubles
- [x] No `default:` in a `switch` over a house enumeration, so a new state is a compile error under `-Wswitch -Werror` — **zero**, from three (`clients/AppWasm.cpp:374` and `clients/SceneRunner.cpp:167` over `Outshine::Phase`, `generators/draw/TreeGrower.cpp:219` over `Architecture`). *The five first written here counted every `default:` label, and two of those five are over a `char` escape and a protobuf wire type* (`core/Json.cpp:142`, `world/OsmVector.cpp:49`) — neither is a house enumeration and both correctly survive, and `sim/tools/hardening_ledger.py` now publishes both numbers beside each other (`defaultOverHouseEnum` 0, `defaultLabels` 2). Held by the compiler rather than by a rule: `-Wall` carries `-Wswitch` and `sim/Makefile`'s `CXX_WARN`/`EMCC_WARN` carry `-Werror`, so a new enumerator now fails to build at all three sites instead of falling into the arm that reports failure
...
- [x] `make gates`: one target running every negative gate and every declared sanitised run, one line per gate, every gate run even after one has fallen, non-zero on any failure (`sim/Makefile`, `GATES`). **Eleven gates, 8 m 24 s**, of which `verify-hardening` 97 s; the seven-gate 6 m 13 s reading was `verify-generators` 14 s · `verify-clients` 0 s · `verify-types` 0 s · `verify-world` 1 s · `verify-counters` 41 s · `verify-refusals` 4 s · `verify-walk-asan` 313 s. Checked adversarially — the treebench refusal reverted gives `FAIL verify-refusals`, `gates: RED`, exit 2, and the other gates still run
...
*Added 2026-08-12 on five rulings from the owner in one session: tests are C++ under `sim/test/` in
...
- [ ] No `static_assert` on the size of a struct containing a pointer or a `size_t`; only structs of exact-width and floating members are pinned — `generators/Body.h` (48 B) and `core/ChunkVtx.h` qualify, `world/terrain`'s `Chunk` correctly does not
- [ ] Exact-width integer types everywhere a number is stored — **183 platform-width declarations** outside `world/terrain` today (`long`, `unsigned long`, `long long` as a declaration; the count excludes casts). `long long` is not wrong, it is at least 64 bits on every target, but the width is the platform's choice and `int64_t` says what it is. **Done when** the count outside the `world/terrain` C ABI is 0, held by a test in the suite — `long` is a keyword, so no type can forbid it and no `#pragma poison` reaches it; this is a rule a check carries
...
- [ ] `world/terrain/` keeps its C-ABI exception and loses its platform coupling: an allocator pair (`alloc`/`free` function pointers) and a diagnostic callback passed in by the caller, no `stdio.h`, no `malloc`. It is shared with `tiles/`, which passes libc's — so the exception survives and the seam moves. This is the same work as § I.17's *no `malloc` outside `core/io/`*, which is stuck on exactly these files
...
- [ ] **wasm is a port destination, not a constraint** — one host implementation among several, reached when SDL3 reaches it. What that ends: the emcc half of `sim/Makefile` stops being load-bearing, `-sINITIAL_MEMORY`/`-sPTHREAD_POOL_SIZE`/`-sABORTING_MALLOC` stop being the engine's frame, and the six emcc-only translation units become one host's implementation rather than a second program. What it must not end: the exact-width discipline of § I.18, which was learned there and is right everywhere
...
- [ ] `sim/test/run.sh` as the harness: discovers, builds, runs each test in its own process, prints one line per test (`PASS`/`FAIL`/`TIMEOUT`/`SIGNAL`/`BUILD`/`SKIP`, name, milliseconds, log path), tabulates, and exits non-zero on anything that is not `PASS`
...
- [ ] The harness's own red demonstrated by a test that is declared to fail — a harness that has never been non-zero is a harness nobody has tested
- [ ] `make gates` becomes one line that calls `sim/test/run.sh`, and the eleven `verify-*` targets are deleted **as they are replaced**, one per step, never in one jump
- [ ] The must-not-compile negatives as ordinary tests that shell out to the compiler — fixtures under `sim/test/negative/fixture/`, one driver per fixture, the toolchain and source root taken from the environment and **refused if absent** rather than defaulted
...
- [ ] The normalisation contract held for every declared species — base at `y = 0`, centred in x/z, height exactly 1 (`generators/draw/TreeMesh.h`), which nothing checks today
