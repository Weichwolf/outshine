...
- [ ] **So the corpus entry DECLARES the asset's state and does not correct it, for two reasons and the second is the stronger.** § I.26's standing rule — *an oracle comparison must contain no repair* — and: **a winding correction would make us non-conforming.** Asserting the near surface at 1.783 m would assert a picture the format forbids for this file. *A repair that moves us away from the specification is not a repair even when it moves us toward the intended picture*
- [ ] **What is ours to record is the incoherence the ray exposed, and it disqualifies one number rather than the case.** The **lit** set is computed from the file's normals (`N·L`) and the **unoccluded** set from the geometry (the ray), and on this asset those are two different surfaces — **0 of 8 043 light-facing vertices are occluded once the normals are flipped.** So **any self-shadowing number from this case means nothing** and the manifest says so; hue, coverage and silhouette are unaffected and keep deciding what they always decided
