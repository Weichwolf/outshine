...
| `MultiUVTest` | colour from `TEXCOORD_0`, logo from `TEXCOORD_1` — a second UV set collapsed onto the first | the logo appears in its own parameterisation | **`TEXCOORD_1` is named in a comment and read nowhere** (`gltf/Types.h:6`); the vertex layout carries one UV (`core/ChunkVtx.h`) |
...
| `VertexColorTest` | `COLOR_0` multiplied into `baseColor` — vertex colours ignored | the "Test" row shows clean checkmarks, no X marks fighting them | **`COLOR_0` is named in a comment and read nowhere** (`gltf/Types.h:6`), same as `TEXCOORD_1`; no layout, no case |
...
| `NormalTangentMirrorTest` | an engine's ability to **use supplied** tangents — the mirrored columns invert when regenerated | the two "Mirror" sets are not inverted | **`TANGENT` does not cross the reader at all** — `gltf/Subject.cpp` reads `POSITION`, `NORMAL` and `TEXCOORD_0` and nothing else (`:205,230`); `git grep TANGENT src/gltf` is empty |
...
| `DirectionalLight` | *"that the BRDF and microfacet model used by a renderer preserves energy"* — an NDF factor that adds light | **no linear pixel above `colour · intensity = [0.9, 0.8, 0.1]`**, and the highlight retains the light's hue `[0.5, 0.444, 0.0556]` | **the linear tap exists** — `Renderer::ReadSceneLinear` (`render/Renderer.h:87`, `.cpp:947`) returns f32, so the instrument is no longer the blocker; **no punctual light** (`git grep KHR_lights_punctual src` is empty) |
