...
- [ ] **So the rule generalises rather than gaining an exception: a binary predicate's disagreement is bounded in the predicate's own geometry.** Camera coverage → screen pixels, against the oracle's filter half-width (§ I.26.14). Light visibility → screen pixels, against **one shadow texel projected to screen**. **Neither is ever a term in codes**, because codes are the unit of appearance and a predicate has none
...
- [ ] **The two answers are not in competition and both are wanted.** The exact ray is for **subject-sized** scenes and gives a zero term; the map is for the **world**, where `T` is orders larger and no ray budget survives. *So the ruling is: the projected-texel bound is the rule and it is written now because the world will need it; the exact ray is measured first because a zero term is better than a bounded one wherever it is affordable*
