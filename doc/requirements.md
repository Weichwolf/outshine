...
- [x] **The names sort for browsing and the order is part of the contract**, not alphabetical accident: `0-oracle.png` · `1-outshine.png` · `2-diff.png`, beside `scene.gltf` and the two `.exr`. The purpose is visual comparison in a file browser, so *oracle first, ours second, difference third* is what the numbering buys *(`test/render/Parity.cpp`, `test/render/coverage/triangle/`)*
...
| `Box With Spaces` | URI-encoded spaces in the texture names — a client that hands a URI to the filesystem undecoded | every declared `uri` resolves to its file | **fails** — `doc/bugs.md` |
...
| `MultipleScenes` | *"the default scene is set to `1` … so viewers should initially show the scene containing the square"* | the drawn coverage is the **square**, not the triangle | reader reads the default scene; **no case** |
| `MeshPrimitiveModes` | seven modes over one hexagon of shared vertices — a topology silently dropped or mis-triangulated | the two triangle topologies draw the same surface **and the same facing**; the five we refuse are refused **by name** | strip and fan draw; **facing unheld** — `doc/bugs.md` |
...
| `OrientationTest` | node rotations as **quaternions** (red/green/blue arrows) and as **matrices** (cyan/magenta/yellow) — *"a misplaced matrix transpose"*, row-major against column-major | **every arrow points at the target of its own colour**; the two arms fail independently | reader carries both forms; **no case** |
| `NegativeScaleTest` | a node with negative scale — winding not flipped by a negative determinant, and normals not flipped | the determinant of the node's **global** transform decides the winding (spec, `Specification.adoc:1734`); the two white spheres are lit alike | **nothing computes a determinant**; culling is off |
...
| `SimpleTexture` | one base-colour texture | coverage · radiance at 1 texel per pixel | **no image decoder in the tree** |
| `TextureCoordinateTest` | *"how XYZ and UV positions relate"*, accessors named by function — a flipped V, a transposed UV | `TopLeft_TEXCOORD_0` samples the upper-left of the image | no image decoder |
| `TextureSettingsTest` | `doubleSided`, and `REPEAT` · `CLAMP_TO_EDGE` · `MIRRORED_REPEAT` | every "Test" cell matches its "Sample pass" cell; **two clamp cells solid green, the rest green checks, no red anywhere** | none |
| `MultiUVTest` | colour from `TEXCOORD_0`, logo from `TEXCOORD_1` — a second UV set collapsed onto the first | the logo appears in its own parameterisation | vertex layout has **no second UV** |
| `TextureEncodingTest` | base colour and emissive **decoded from sRGB**, metallic-roughness **passed as-is**, and embedded **gamma values and ICC profiles ignored as the spec requires** | the four columns of a row render identically | no image decoder; no rule stated anywhere |
...
| `VertexColorTest` | `COLOR_0` multiplied into `baseColor` — vertex colours ignored | the "Test" row shows clean checkmarks, no X marks fighting them | `COLOR_0` crosses the reader; no layout, no case |
| `NormalTangentTest` | an engine's ability to **generate** tangents at runtime — no `TANGENT` in the file | horizon reflections level across every cell | no tangent generation |
| `NormalTangentMirrorTest` | an engine's ability to **use supplied** tangents — the mirrored columns invert when regenerated | the two "Mirror" sets are not inverted | `TANGENT` crosses the reader; nothing uses it |
| `AlphaBlendModeTest` | `OPAQUE` ignoring alpha entirely, `BLEND`, `MASK` with `alphaCutoff` at `0.25`/default/`0.75` — alpha used in opaque mode, **premultiplied alpha**, cutoff ignored, default cutoff not `0.5` | green checks at every label; no red X; **no black box beside "Opaque"** | `SurfaceState` has **no `BLEND`** and **no per-material cutoff** |
| `DirectionalLight` | *"that the BRDF and microfacet model used by a renderer preserves energy"* — an NDF factor that adds light | **no linear pixel above `colour · intensity = [0.9, 0.8, 0.1]`**, and the highlight retains the light's hue `[0.5, 0.444, 0.0556]` | no punctual light, no linear tap |
| `PointLightIntensityTest` | *"the `color` property acts as a wavelength-specific multiplier"* — colour multiplied into intensity | **Red + Green + Blue ≡ White per channel**, Gray strictly dimmer, tone mapping off or linear | as above |
| `EmissiveStrengthTest` | `KHR_materials_emissive_strength` — the extension ignored leaves all five cubes equal | **linear ratio 2.000 between neighbours**, measured before any clipping | no `extensions` object parsed |
...
| `SciFiHelmet` | one real asset's material stack | coverage · radiance | none |
| `ABeautifulGame` | scene scale with transmission and volume | coverage · depth | none |
...
- [ ] **`alphaCutoff` is a per-material number with the spec's default, and today it is one `[SET]` constant for the whole engine.** `src/core/SurfaceState.h:39` carries `CoverageCut_ = 0.5f` — the right default by the spec (*"`alphaCutoff` … names `0.5` as the default"*, and `AlphaBlendModeTest` fails an engine that ignores a declared one) but reached by our own leaf-edge argument rather than by citation, and **not settable per material**. `AlphaBlendModeTest`'s `0.25` and `0.75` columns cannot pass until it is
- [ ] **`alphaMode: BLEND` has no spelling in this engine and `SurfaceState` derives a mode from a magnitude.** `StateOf` (`src/core/SurfaceState.h:42-58`) sets `Blends_` only for `Refractive`, and maps `Coverage < 1.0f` to `Masked` — so a `BLEND` material becomes a binary cut and an `OPAQUE` material with a base-colour alpha below 1 becomes masked, where the spec says *"`OPAQUE` — the rendered output is fully opaque and any alpha value is ignored"* (`Specification.adoc:2178`). **The mode is a three-valued property of the material and a float cannot carry it** (`Enum.2`): what the bridge needs is `alphaMode` as an enumeration crossing from the file, with `Coverage` remaining the *quantity* it samples. § I.26.6's row claiming alpha `MASK` and `BLEND` are already expressed by `Material::Coverage` is true of `MASK` alone
- [ ] **A URI is percent-decoded before it reaches the filesystem** — *"Reserved characters … **MUST** be percent-encoded"* (`Specification.adoc:550`), which is a requirement on the writer and therefore a decoding obligation on us. `Box With Spaces` is the asset that exposes it and it exposes it in the **image** URIs specifically, while its buffer URI carries literal spaces — so a reader that concatenates works today and breaks at the first texture (`doc/bugs.md`)
- [ ] **`asset.minVersion` is read and refused by name when it exceeds 2.0**, the same door as `extensionsRequired` one line away in the same function (`doc/bugs.md`)
