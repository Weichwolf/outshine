...
- [ ] **The three things that must exist together, because each is useless without the other two.** A **camera path** — declared, deterministic, and moving, since `CLAUDE.md` says *performance is a distribution over a moving camera, p50/p95/p99, never a mean* and there is **no moving camera anywhere in the suite** · a **per-frame clock** whose floor is published beside its result · a **scene worth measuring**, which today does not exist because nothing draws a landscape
...
- [ ] Readback of colour and depth, and a PNG writer (`src/clients/Png.cpp`, `src/clients/FileArtifacts.cpp`, held by `Makefile` `verify-still`, which fails if a run writes no still). *Posting the artefacts to `fb-sim` was the second half of this line and `b83285f` deleted `ServerArtifacts.{h,cpp}` with the collector; the artefacts are written to `OUTSHINE_OUT` and nowhere else.* — **unbuilt**: the client that carried this was deleted with the browser-era clients; the line is scope again
...
- [ ] **RGBA, not RGB.** Straight alpha is on both sides and the channel is load-bearing: an empty render scores **50** differing pixels under RGB-only against **46 151** under RGBA, because the hole was 46 101 px wide and black-on-black in colour. *And the consequence is deliberate — our alpha is `covered(sceneDepth)`, which `doc/bugs.md` records as wrong for a blended surface over nothing, so including alpha makes a filed defect finally score. An instrument reaching a known defect is the instrument working*
...
- [ ] **The default bound is ZERO and a term enters only where a mechanism puts it there.** *This is the honest reading of "a sensible threshold": the threshold is not chosen, it is the sum of what is known to differ, and a case with nothing in its path gets nothing.* **29 of 33 render cases are already bit-identical**, so zero is measured and not aspirational

| what is in the path | term | bound |
|---|---|---|
| nothing — flat emission, coverage, unfiltered lit | f32 arithmetic order only: `γₙ = n·u/(1 − n·u)`, **5.96e-6 relative** at `n = 100` (§ I.26.13, Higham) — below one part in 10⁵ of a code | **0** |
| a **sampler** — `filter-bounded` | sub-texel weight snapping, `255 · 2^-(n+1)`; `n` is **not published for Metal** and is one render to measure (§ I.26.14) | the weight term alone, **blocked on that run** |
...
- [ ] **Generator — abstract, and it passes all three.** Five implementations in the tree; `Ground`'s unspellable list is already ticked under § I.9; a scenario or a mod registers one by rank at run time (`generators/GeneratorSet.h`). **`generators/Generator.h` is correct as it stands and this section changes nothing about it**
- [ ] **Compositor — abstract, and it passes all three.** Four by the owner's enumeration, each with a distinct aggregation problem — terrain (tile adjacency, skirts), forest (density and species from land cover), city (footprints, street alignment), traffic (roads, per-frame). *"A compositor knows what a pipeline is"* has no spelling because its reply is a `DrawList`. The set is open: traffic in a walk scene and no traffic in a studio scene is a run-time choice
...
- [ ] Plinth and base course as a distinct band — `src/render/stages/BuildingDraw.cpp` — a shading band, not geometry — **the implementation named here was deleted with the WebGPU renderer at `0161f88`**; the stage survives in the catalogue and `Renderer::Executable` refuses it by name (`render/Renderer.cpp:102-127`), so this is scope again and not a silent loss
...
- [ ] Window rhythm: bay spacing, alignment between storeys, a wider or narrower ground floor — `src/render/stages/BuildingDraw.cpp` — **the implementation named here was deleted with the WebGPU renderer at `0161f88`**; the stage survives in the catalogue and `Renderer::Executable` refuses it by name (`render/Renderer.cpp:102-127`), so this is scope again and not a silent loss
- [ ] Window as an opening with a reveal depth, not a decal — `src/render/stages/BuildingDraw.cpp` — a marched box recess — **the implementation named here was deleted with the WebGPU renderer at `0161f88`**; the stage survives in the catalogue and `Renderer::Executable` refuses it by name (`render/Renderer.cpp:102-127`), so this is scope again and not a silent loss
...
- [ ] Shop window at ground floor, full height — `src/render/stages/BuildingDraw.cpp` — **the implementation named here was deleted with the WebGPU renderer at `0161f88`**; the stage survives in the catalogue and `Renderer::Executable` refuses it by name (`render/Renderer.cpp:102-127`), so this is scope again and not a silent loss
- [ ] Roller shutter, louvre shutter, folding shutter
- [ ] Blind or curtain visible behind the glass — `src/render/stages/BuildingDraw.cpp` — **the implementation named here was deleted with the WebGPU renderer at `0161f88`**; the stage survives in the catalogue and `Renderer::Executable` refuses it by name (`render/Renderer.cpp:102-127`), so this is scope again and not a silent loss
- [ ] Glass: reflectance and a dark interior at the comparison rung; a lit room only at night — `src/render/stages/BuildingDraw.cpp` — Schlick; the lit room is not built — **the implementation named here was deleted with the WebGPU renderer at `0161f88`**; the stage survives in the catalogue and `Renderer::Executable` refuses it by name (`render/Renderer.cpp:102-127`), so this is scope again and not a silent loss
