...
| **`FBSeekerKind::SemiActiveLaser`** and the Paveway PURSUIT law with a rate-stabilised bang-bang relay, **per channel** — expressly not proportional navigation | `FBMissileGuidance::LaserCommand` · `assets/aircraft/gbu12/` | `lgb-designate` (**14.45 m**, `C30`) `lgb-lase-broken` (1 906 m) `lgb-lase-restored` (**15.36 m**, `C30`) |
...
| `C29` | **The MiG-29's director releases 0.194 s EARLY — the opposite sign to the F-16's defect, and the cause is not isolated.** [MESS, `o3-01-unopposed`] `stores DELIVERY aimLongM` = **−45.5 m**, i.e. short, at 234 m/s = 0.194 s early; `openLoopAlongM` −70.3 m at the pull. Found by ASSUMING the F-16's separation-tick defect transferred (the store crosses the same queue) and being refuted: leading the countdown by one tick took the same delivery to **−68.2 m** and five `o3-*` missions from exit 0 to exit 3. The change was reverted and `o3-01-unopposed` flies byte-identical to before. **The magnitude ~0.19 s is the same as `C28`'s**, on a different path and with the opposite sign — whether that is one cause or two is unknown, and a number without an isolated cause is not a repair. Booked, not fixed | this file; owner `modules/mig29/FBMig29Director` |
| `C30` | **The 52.78 m was two thirds a relay defect and one third a delivery no Paveway II user flies. The relay half is REPAIRED (52.78 → 14.45 m); the delivery half is booked below and is not the law's.** <br>**The defect.** A test on the TOTAL misalignment sat over the two channels — `if (errDeg > kLaserDeadBandDeg)` — and vetoed both whenever the raw angle dipped inside the same 1.5° the channels test their LED signals against. Two different quantities, one threshold: it fired on a state neither channel could see and silenced the pitch channel while its own signal still called for the stop. The cost was the FREQUENCY it imposed, not the vetoed ticks. Measured on a constant-canard step, the deck's short period is **ζ ≈ 0.14, period ≈ 5 s, 12 s to settle at α = 20°** — so dropping the relay out of its limit cycle roughly once per period re-excited the mode every time and the cycle sat ON the airframe instead of an order above it. Per channel (**one `if` deleted, not one constant touched**) the cycle moves to ~3 Hz and the airframe integrates it: <br>`switches/26 s 48 → 156` · `α −7.8…25.8 → 9.2…25.7°` · `standing pursuit error 3.92 → 3.00°` · `aimErrM 52.78 → 14.45 m`. `lgb-lase-restored` 42.96 → 15.36 m. `lgb-lase-broken` 1 902.67 → 1 906.35 m (+0.19 %, the guided segment before the lase stops) — **still the ballistic dump, so the illumination proof holds.** 293 of 296 missions byte-identical, all 296 exit codes unchanged, eight f22 sorties byte-identical. <br>**What the residual IS, bounded by an oracle rather than asserted.** The same deck and geometry under a smooth gravity-compensated pursuit autopilot (diagnostic, reverted) hits **5.16 m long**; without the gravity term **10.60 m short**. So the bang-bang now costs **3.9 m against a smooth controller** — [T4]'s *"relatively little effect on accuracy"* — and the remaining ~10 m is pure-pursuit lag, which is what §Knowledge 4 says a pursuit law costs and is not a defect. **Two repairs were tried and refuted by measurement, both reverted:** the rate lead deleted → **139.07 m** (the relay saturates and the airframe rings free), and the lead taken on the detector error's own rate instead of the gyros → **109.04 m** (ė is ~0.05°/s, numerically negligible, so the relay never switches). A gravity bias on the relay's null was **refused**: its magnitude needs a horizon nothing in the file derives, and choosing one is the tuning this file forbids. <br>**One residual is left standing and named:** the pitch channel commands `−1` on **0.000** of the terminal phase, before the repair and after it, so the miss is one-sided (short in all 15 runs measured here) where [T4] says the fins *"deflect until over-correction and then deflect back in the opposite direction"* and [T2] says *"short **or** long"*. Under gravity the velocity never gets above the line of sight, so the relay never needs the other stop. Whether that is the dead band's null or the deck is not isolated, and a number without an isolated cause is not a repair | this file; owner `modules/missile/FBMissileGuidance::LaserCommand` |
| `C28` | **`sms RELEASE_SOLUTION` is not a report about the release it names — it is 0.2 s older, and therefore 44 m wrong.** [MESS] `predLat`/`predLon` against the store's own back-solved ground crossing: **+44.1 m along-track, −0.1 m cross** on `attack-ccrp`, `attack-ccip` AND `cbu87-footprint` — three stores, one number — and +55.6 / −14.0 m on the turning `w2-01-dome`. The predicted TIME OF FLIGHT is right to 0.01 s (10.0898 vs 10.0996 s), so the ballistics are sound and the error is purely the RELEASE POINT. **Isolated by reading, not guessed:** `FBF16FireControl::SolveGroundAttack` integrates from `own.lat/lon` of the running tick, i.e. it answers "where would it land if I released NOW" — which is exactly right for the CCIP pipper and for `AgTimeToReleaseS` (this round's ground-crossing measurement confirms the cue is correct to inside one tick). The two terms between that answer and the truth are both already on the line: the solution's own age `solAgeS` = 0.1 s (the SMS logs it) plus `kSeparationDelayS` = 0.1 s → 0.2 s × ~220 m/s = 44 m. **So the BUS block is right and the LOG LINE lies**, and it lies by naming a delivery it did not describe. Consequence, measured: `aimMissM` moves the WRONG WAY under a correct fix — 12.1 → 30.5 m while the true miss went 40.4 → 19.5 m. **The same number is also mis-explained in the code**: `FBOrdnance.cpp` logs it as `predErrM` and attributed it to *"stored table vs. the model's aerodynamics"* — falsified, because `tofErrS` is 0.01 s on all three cells while the error is 44.1 m along and 0.1 m across; a table error shows in the TIME. The comment is corrected in this round. **Not repaired here** (one thing per round, and this round's repair is the cue); the repair is to extrapolate the logged solution forward by those two declared terms. It is why `pilot.md` §4.2's previous round read its own result as "~0" | this file; owner `modules/f16/FBF16FireControl` |
...
| N11 | **No LGB delivery doctrine: FlightBox lases from release, and every source says not to** | The lase window is the one delivery parameter a Paveway II has, and it is sourced three ways. [T4] *Paveway*: bang-bang gives *"a noticeable wobble. This had relatively little effect on accuracy, but expends energy quickly … As a consequence, most users release Paveway I and II weapons in a ballistic trajectory, **activating the laser designator only late in the weapon's flight** to refine the impact point."* [T2] BMS TO 1F-16CMAM-34-1-1 §4.2.8: *"lasing should start **12 sec before impact** (to minimize the movements of the 'bang-bang' control, and consequently **the bomb falling short or long**)."* [T2] ED via [`modules/f16/weapons.md`](modules/f16/weapons.md): *"lase no later than **8–12 seconds** before impact."* `lgb-designate` lases **33.8 s**, and `FBStoresSystem`'s `brief_lase_s` is the only way to express a window at all — there is no doctrine, no pilot behaviour and no `.fbm` grammar for "lase late". **Measured on `lgb-designate`'s geometry, holding everything but the lase start** (`set brief_lase_s 0.1 <t>`, current binary): <br>`33.8 s → 14.40 m` · `24.7 → 12.89` · `16.9 → 13.81` · **`13.3 → 9.52`** · `11.6 → 25.29` · `8.9 → 196.4` · `4.9 → 621.6` <br>The sourced window is where the file's own 6–9 m band is reached, and the cliff below it is honest rather than a defect: at 8.9 s the relay is on the +1 stop for **89 of 90 guided ticks** — saturated, flying out the ~1.9 km ballistic error the mission deliberately creates, which is a correction no Paveway II makes. **Not repaired here**: `lgb-designate`'s header forbids moving its geometry, and the missions are the acceptance surface of the round that wrote them. A doctrine round owns this |
...
**The wobble is the law, not a defect of it — and its price is RANGE, not accuracy.** [T4] states both
halves: the CAS *"commands large canard deflections … resulting in a noticeable wobble. This had relatively
little effect on accuracy, but expends energy quickly, limiting effective range."* A second [T4] source
gives the shape: *"the fins deflect until over-correction and then deflect back in the opposite direction,
creating a sinusoidal type of flight path."* Both are reproduced, and `C30` measures the price where the
sources put it — the bang-bang costs **3.9 m** against a smooth controller on the same deck and geometry.

**Which is why the delivery, not the law, carries the accuracy.** The same [T4] paragraph ends: *"most
users release Paveway I and II weapons in a ballistic trajectory, activating the laser designator only late
in the weapon's flight."* Two [T2] sources put a number on "late" — 8–12 s before impact — and the BMS one
gives the mechanism verbatim: minimising bang-bang movement, *"and consequently the bomb falling short or
long."* FlightBox has no such doctrine and lases from release; the characteristic curve and the refusal are
`N11`.
...
| [T4] | [Wikipedia — AGM-88 HARM](https://en.wikipedia.org/wiki/AGM-88_HARM) · [Mark 84 bomb](https://en.wikipedia.org/wiki/Mark_84_bomb) · [GBU-10 Paveway II](https://en.wikipedia.org/wiki/GBU-10_Paveway_II) · [GBU-12 Paveway II](https://en.wikipedia.org/wiki/GBU-12_Paveway_II) · [Paveway](https://en.wikipedia.org/wiki/Paveway) (the bang-bang wobble, its energy cost and the late-lase practice — `C30`/`N11`) · [CBU-87 Combined Effects Munition](https://en.wikipedia.org/wiki/CBU-87_Combined_Effects_Munition) · [Hydra 70](https://en.wikipedia.org/wiki/Hydra_70) · [FAB-500](https://en.wikipedia.org/wiki/FAB-500) · [FAB-250](https://en.wikipedia.org/wiki/FAB-250) |
