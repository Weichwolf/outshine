...
| **`FBSeekerKind::SemiActiveLaser`** and the Paveway PURSUIT law with a rate-stabilised bang-bang relay — expressly not proportional navigation | `FBMissileGuidance::LaserCommand` · `assets/aircraft/gbu12/` | `lgb-designate` (**52.78 m**, `C30`) `lgb-lase-broken` (1 903 m) `lgb-lase-restored` (**42.96 m**, `C30`) |
...
| `C30` | **The GBU-12's pursuit law falls 52.78 m short of a correctly designated spot, and the 3.9 m this file used to publish was never a property of the weapon.** The spot's elevation is `FBReleaseSolution::ImpactElevM`, i.e. the CCRP impact plane — and until the steerpoint carried its own briefed terrain that plane was the ground under the SHOOTER. On `lgb-designate`'s geometry the shooter sat 30.45 m above the target, so the spot hung 30.45 m in the air and the round, still descending toward it, crossed the real surface further downrange. **Causality measured, not inferred** — a temporary offset on `FBStoresSystem.cpp`'s spot elevation, reverted, gym binary byte-identical afterwards: <br>`+30.45 m → aimErrM 2.33 m` · `+15.00 m → 28.01 m` · `0 m (correct) → 52.78 m` <br>≈ **1.66 m of miss per metre of spot error**, zero crossing at **+31.8 m** — the old figure was one compensating error cancelling another to within 2 m. `lgb-lase-restored` moves 11.37 → 42.96 m by the same mechanism; `lgb-lase-broken` is the ballistic dump and moves 1 894 → 1 903 m, which is noise. **Both mission files stay RED**: the round genuinely does not reach the 6–9 m band, and `lgb-designate`'s declared `aimErrM < 10 m` is now a measurement of the guidance law rather than of the terrain under the release point. The repair is the law's energy/authority, not the spot | this file; owner `modules/missile/FBMissileGuidance::LaserCommand` |
