...
| **D3c** | **BUILT (2026-07-31): the pilot manages his own emission, and G5 is a key.** The rule is *"radiate ⟺ the nearest point my picture holds is within `emcon_frac × EmconRadiateNm()`, OR I have no other picture at all"* — a lone fighter cannot go silent, because then he goes blind, which is D3's own sentence made structural. Two new virtuals, both defaulting to "this aircraft does not manage emission" (`SilentRadarModeOrdinal() = -1`, `EmconRadiateNm() = 0`), so an airframe that overrides nothing is a compile-time no-op. The F-16 overrides them with `FBF16FcrMode::Off` and the FCR's own 40 nm search gate — **no new capability is claimed**: its module still REJECTS `RadarEmission` as `NotImplemented`, and `Off` sets `Active = false`, which `FBRadarSystem` turns into `Radiating = false` and an EMPTY emitter signature | [MESS] the gene bites where E-22 demanded it: on `w3-09-saturation:f16` two of three EMCON levers move the outcome class, `(16, 10)` → `(20, 12)`. **The pre-round shape is reachable**: `emcon_frac = 3.0` reproduces `(20, 12)`, the value measured before the gene existed. Behaviour: **20 of 251 missions moved, one exit code** (`w4-10-allied-force` 2 → 3); eight missions lose EVERY `FAIL`, two lose `SUCCESS`es. Determinism over `--threads 1/2/4`: identical MD5 **AMENDED 2026-08-03.** The rule and its two virtuals stand unchanged and nothing here was re-designed, but D3c's own acceptance numbers were taken on a build in which going silent could not be undone: `pilot/FBPilot` guarded its `RadarMode` posts with `Radar.H.Readable()`, which `FBRadarSystem` clears as soon as nothing radiates ([`pilot.md`](pilot.md) §7.6b). So *"20 of 251 missions moved"* and the `w3-09-saturation` `(16, 10) → (20, 12)` step are measurements of **the rule plus a one-way door**, and the `emcon_frac = 3.0` reproduction succeeded because that value keeps the door from ever shutting. The gene's own behaviour is now a real duty cycle — on `sat-02-picture-split` an F-16 goes quiet for 1.0 s and then for 30.6 s and comes back both times — and every G5 number in this tree needs re-taking. |
...
| # | Thing | Known from |
|---|---|---|
| **D3e** | **CLOSED (2026-08-04, `E20`) — D3d repaired, and the repair needed TWO seams rather than the two numbers it asked for.** The two numbers are on `FBMig29Pilot`: the silent state is the **PUR-31 at DUMMY** (`DCS-EA p.63`, *"does not radiate"*; `radar-sensors.md` §2.1 maps it to EMCON itself) and never OFF, because OFF drops the set's power and `Powered == false` is the one state [`pilot.md`](pilot.md) §7.6b proves a pilot cannot come back through; and `EmconRadiateNm` is the N019's OWN documented reach, `FBMig29Radar::kSearchRangeM` = 50 km = **27.0 nm** (T4 §7.1, the lower end of the RCS-qualified 50–70 km band) against the F-16's 40.0 — this jet's number, not a copied one. **But the two numbers alone changed not one bit**, and that is the finding: (1) the EMCON actuation posted `RadarMode`, an axis this jet's mode selector already uses for search-against-close-combat, so the switch became a hook (`FBEmissionControl`, §7.6d); (2) `EmconSilent_` required a cooperative report, i.e. the rule as coded said *an airframe with no datalink may never be quiet* — the exact opposite of this airframe's doctrine. `BriefedPictureRangeM` lets the CONTROLLER'S last call be that picture, with the controller's own briefed cadence as its lifetime. [MESS, bit level, SHA-256 over the whole MiG side] the G5 band now produces **4 distinct trajectories on `sat-02`/`sat-07` and 6 on `sat-04`** over a 10-point sweep, and the step positions are exactly `f × 27.0 nm` crossing the briefed GCI ranges (120 / 90 / 60 km ↔ f = 2.4 / 1.8 / 1.2). **The pre-round shape is reachable**: at `f ≥ 2.5` the repaired jet is byte-identical to the pre-repair binary on all three cells | `E20` |
| **D3d** | *(REPAIRED by D3e above; kept with its measurement.)* **G5 is a SINGLE-MODULE gene, and every doctrine number this tree has published about it is an F-16 number.** [MESS, [`doctrine-evolution.md`](doctrine-evolution.md) `E19`] `FBPilot::SilentRadarModeOrdinal()`/`EmconRadiateNm()` default to −1/0.0 and the whole EMCON block is gated on both (`FBPilot.cpp:1540`); the only file in the tree that overrides them is `FBF16Pilot.h:71–72`. D3c states the default as a design property — *"ein Modul, das nichts überschreibt, verhält sich Bit für Bit wie vor D3c"* — and that is exactly what the MiG-29 does: all three `emcon-*` levers spliced into a MiG side are **bit-identical over the whole telemetry** on `sat-02`, `sat-04` and `sat-07`. Nothing is wrong in the code; what is missing is that the arena, the fitness and four rounds of doctrine graded a gene only one of three flying modules can carry. **Owed:** either an N019 emission ordinal + search range on `FBMig29Pilot` (the switch and the range both exist — `n019_emission off\|dummy\|illum`), or a declared per-module reachability the tooling can read | `E19` |
