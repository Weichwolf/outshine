...
**Implication for `FBF16Pilot`**: our measured on-speed CAS for 11° AoA is a **model
...
|---|---|---|
| Approach → flare → roll-out as a procedure | **built** — pilot phases `Approach`/`Flare`/`Rollout` | [`../../pilot.md`](../../pilot.md) §3 |
| Approach speed | **built, but as a speed, not an AoA**: `ApproachSpeedKt` = 165 kt `[MESS]` — the trimmed pinned model holds 11.0° AoA at 164.9 KCAS, gear down, ~40 % fuel | [`module.md`](module.md) §3.2 |
...
| Localizer-style lateral tracking | **built** — `FBAutopilot` COURSE guidance (a flown, measured approach law), not an ILS receiver | [`../flightbox/sim/systems.md`](../../systems.md) §2 |
...
| Crosswind landing technique | **not implemented** — and as of the state this table was written against, unreachable: no wind field, JSBSim's `FGWinds` unwired. The weather round (roadmap R4) is changing exactly that; **verify against `doc/` before relying on this line** | [`../flightbox/roadmap.md`](../../roadmap.md) R4 |
| Go-around / ALOW-as-decision-height | **not implemented** — ALOW exists as a warning (`FBF16Ufc` + `FBWarningSystem`) but is not wired to any missed-approach logic | [`../flightbox/sim/systems.md`](../../systems.md) §6 |
...
- *Partially:* the approach is flown to a **fixed CAS**, not to the documented **AoA band 11–13°**;
  ...
  measured and open — [`../../pilot.md`](../../pilot.md) Gaps 2.5.
