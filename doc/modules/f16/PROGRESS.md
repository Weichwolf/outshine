...
- `doc/DCS MiG-29 Flight Manual EN.pdf` and `doc/DCS MiG-29A Early Access Manual EN.pdf` are present
  ...
  `doc/modules/mig29/` knowledge base, the same two-tier approach (official EA manual = primary system-logic
...
onto the same `## Spec` / `## State` / `## Gaps` / `## Knowledge` form that `doc/` uses, so
...
| `## State` | what FlightBox implements of it — brief, table-shaped, linking into `doc/`; never a copy of that content. **Newly written this pass**, established by reading `doc/modules/f16/module.md`, `sim/sensors.md`, `sim/systems.md`, `sim/weapons-and-damage.md`, `sim/pilot-ai.md`, `sim/fdm.md`, `aircraft/stores.md`, `clients/clients.md`, `journal.md` and `roadmap.md` — not guessed. |
...
the loader skill (states what the four sections mean here; merged into the single
`.claude/skills/flightbox/SKILL.md` in the Phase-3 mirror rebuild).

### COVERAGE (Pass 5): COMPLETE for its own scope
Every topic file carries the four sections; every file's State is backed by a named `doc/`
