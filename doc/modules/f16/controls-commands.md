...
|---|---|---|
| propose → commit/reject as the command shape | **built** — `core/FBAvionicsCommand.h`: a command is `{target, proposed value}`, the answer an acknowledgement `{result, reason}` | [`../flightbox/sim/core.md`](../../core.md) |
...
| Effect-side vs. command-side precondition (ALOW set always succeeds; the *warning* needs a powered CARA) | **built** — the radar altimeter is the reference case: unpowered it publishes no 0 ft, it invalidates its block, and the warning reports INHIBITED | [`../flightbox/sim/systems.md`](../../systems.md) §5–§6 |
| Hardware-precedence lockout (weight on wheels, master arm) | **built** — `hardware_precedence` on weapon release and gun trigger | [`../../weapons.md`](../../weapons.md) §2–§3 |
...
| The pilot as a bus client | **built and exclusive** — the autonomous pilot holds no system pointers; what he enters in flight is his brief (`brief_*` mission lines), one input per decision tick, in its latency class, with the risk of rejection | [`../../pilot.md`](../../pilot.md) §2 |
| `WeaponSelect` | **deliberately `NotImplemented`** — the jet has it, FlightBox does not, and the command says so instead of silently succeeding | [`module.md`](module.md) Gaps 4 |
