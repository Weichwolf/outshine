...
Four source documents, kept distinguishable in every file (cite the tag, not just a page number):
...
- **BMS-34** = `doc/TO 1F-16CMAM-34-1-1 BMS.pdf` (Falcon BMS 4.38, 669 pp) — *Avionics and Nonnuclear
  Weapons Delivery*. **BMS-1** = `doc/TO 1F-16CMAM-1 BMS.pdf` (404 pp) — *Flight Manual*. Added in a
  sixth pass to [`hud-symbology.md`](hud-symbology.md), [`cockpit-displays.md`](cockpit-displays.md)
  and [`radar-sensors.md`](radar-sensors.md). These are the only sources in this tree that publish
  **symbol geometry in milliradians**, the **MFD page field maps**, the **ICP/DED entry limits** and
  the **ground-map (GM/GMT/SEA, EXP/DBS) spec**. They are written in USAF tech-order style but
  **document a SIMULATOR, not the factory jet** — they carry their own "not implemented" markers,
  which are reproduced verbatim, and they contradict the ED guide in five recorded places
  (PROGRESS.md, Pass 6). Cite tags `[BMS-34 p.NNN]` / `[BMS-1 p.NNN]`.
...
Every topic file below carries the **same four sections as [`doc/`](../../INDEX.md)**
— with the meaning a *reference* base gives them (this base documents the real jet; `doc/`
...
| `## State` | what **FlightBox** implements of it — a few honest lines with links into `doc/`, never a copy of that content | you need to know whether the thing exists in the simulator |
...
Since the Phase-3 mirror rebuild this directory holds BOTH sides of the F-16: the reference base below
(the **real** jet) and, in the same directory, [`module.md`](module.md) — what **FlightBox** implements
of it (composition, cadence, command router, every override with its numbers and their provenance, the
HUD symbology implementation). The `## State` sections here link into it; they never duplicate it.
...
| [cockpit-displays.md](cockpit-displays.md) | ICP/UFC, DED pages, MFD/OSB, instruments, caution/warning lights — FULL on ICP/UFC/DED and MFD from ED, plus the ED analog-gauge detail (AoA bands, VVI, ADI, Caution Light Panel triggers) — **and now two BMS Dash-34 addenda: the 13-format MFD set with the master-menu and uniqueness rules, the four interaction idioms, the MFDS symbol plate and occlusion model, the whole HSD page (ranges, rings, CPL/DCPL, EXP1/2, freeze, CNTL options), DTE/TEST/FLCS/TCN formats, and the ICP/DED chapter (16 controls, the Dobber, the full entry-parameter range table, the LIST/MISC map, the DED page inventory with BMS status, CNI/STPT/CRUS/TIME/T-ILS/BULL/MAN, fuel warnings)**; Aux/Left/Right Console subsections remain the residual gap | Chuck Part 3, 13–83; ED p.43–81 (partial), p.97–127 (full); BMS-34 pp.63–94, 127–159 |
...
| [radar-sensors.md](radar-sensors.md) | FCR A-A/A-G/A-Sea modes, TGP, HMCS — exact ED scan geometry/timing/thresholds (ACM angles, CRM state machine, TWS/SAM/STT transitions, NCTR gates, GM/GMT/SEA/MTR parameters, TGP zoom/track) — **and now the BMS Dash-34 FCR PAGE, field by field: the OSB frame, the A-A B-scope page and its expanded target-data block, and the A-G ground map (GM/SEA/GMT/FTT/AGR, EXP 4:1 / DBS1 8:1 / DBS2 64:1, expansion cues, range-mark table, gain, freeze, snowplow, sighting rotary) — the spec a radar-map implementation is built against** | Chuck Part 10, 161–312; ED p.342–523; BMS-34 pp.190–222, 226–241 |
...
| [hud-symbology.md](hud-symbology.md) | **near-full** for flight/nav symbology; weapon symbology is drawn with FlightBox's own geometry. The old "source-silent on the TD box" note is **obsolete** — BMS-34 publishes the sizes; the implementation has not yet been rebuilt against them |
| [radar-sensors.md](radar-sensors.md) | **FCR only**: CRM + four ACM sub-modes + STT, designation, emission, chaff notch, IFF Mode 4. No SAM/TWS/DTT, no A-G/A-Sea, no TGP/HMCS/HTS — the A-G ground-map SPEC now exists (§R3) but nothing is built against it |
