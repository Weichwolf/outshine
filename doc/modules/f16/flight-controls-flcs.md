...
| FBW outer loop (rate/attitude PIDs onto `*-cmd-norm`), F-16 gain preset | **built** — `FBFlightControl` + `FBFlightControl::F16()` | [`../flightbox/sim/systems.md`](../../systems.md) §3 |
...
| Damage on the FLCS | **built as authority, not as law**: a degraded FLCS scales the commanded deflections (one of two hydraulic systems), a failed one removes them — the FLCS keeps commanding, the aircraft stops answering | [`../../weapons.md`](../../weapons.md) §8 |
...
- *Partially:* the autopilot — FlightBox has guidance modes, but not *these* modes, and none of the
  ...
  silently binds the outer loop to 100 Hz ([`../flightbox/sim/systems.md`](../../systems.md)
  Gaps).
