...
|---|---|---|
| TMS forward (designate/lock) | `FBRadarSystem::Designate()` via an avionics command — value = the published track number, 0 releases | [`../flightbox/sim/sensors.md`](../../sensors.md) §4 |
| Trigger (gun) | `GunTrigger` command, value = trigger-hold duration | [`../../weapons.md`](../../weapons.md) §3 |
| Pickle (weapon release) | `WeaponRelease` command; rejected on master-arm/weight-on-wheels | same, §2 |
| CMS (countermeasures) | `CmDispense` / `CmConsent` / `CmdsMode` commands | [`../flightbox/sim/sensors.md`](../../sensors.md) §6 |
| Speedbrake, gear, NWS, wheel brakes, throttle | `FBAirframeControls` channels | [`../flightbox/sim/systems.md`](../../systems.md) §9 |
...
| `WeaponSelect` | deliberately answered `NotImplemented` by the command bus — the jet has it, FlightBox does not, and the command says so instead of silently succeeding | [`module.md`](module.md) Gaps 4 |
