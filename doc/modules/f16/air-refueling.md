...
|---|---|
| Fuel quantity as a simulated, changeable state | **built** — `FBFdm::Get/SetFuel*` drives `FGPropulsion` per tank or as a total, distributed proportionally to the model's tank capacities; running dry is JSBSim's own physics ([`../flightbox/sim/fdm.md`](../../fdm.md) §11) |
| BINGO as a decision quantity | **built** — the warning block carries it, and the intercept pilot reads it before pressing on ([`../../pilot.md`](../../pilot.md) §7) |
