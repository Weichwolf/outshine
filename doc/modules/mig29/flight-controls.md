...
**Contrast with the F-16 baseline (`doc/modules/f16/flight-controls-flcs.md`)**: the Viper's FLCS *is* the
...
|---|---|---|---|
| **DAMPER** (ДЕМП) | Damps short-period oscillations in all three axes | Engages **automatically whenever any other AFCS mode is selected** | "In most cases of operations it must be enabled" (`DCS-EA p.64`). **BUILT** as `FBFlightControl::Mig29()`'s `KqDamp`/`KpDampRoll`, and it binds on the `Manual` path as well as the FLCS one — it is a device of the aeroplane, not an autopilot mode, and `DCS-FM p.34` says so directly ("improved stability and controllability … **with manual control** over the entire operational … range"). It bound FLCS-only until the round that measured what that costs: the MiG flew every close engagement undamped and CFITed out of them ([`../../pilot.md`](../../pilot.md) §5.10a). The YAW axis of the three is deliberately absent — this airframe's rudder branch is measured OFF (`KNy = KNyi = 0`, departure at t = 28 s with the generic gains) |
