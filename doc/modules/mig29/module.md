...
| **4h** | **The control-law half is CLOSED; the acquisition half remains.** (b) DEPARTURE — **fixed this round.** The close-combat law departed this airframe (nose-on 8 km merge: roll −150°, 29° α, stall/mush KO at t=24.7) because the BFM law commands raw stick and the deck has no FLCS to hold α or roll rate. Four measured, airframe-scoped screws close it ([`../../pilot.md`](../../pilot.md) §5.10, [`../../duels.md`](../../duels.md) D1): the Manual-path `PitchStickMax` cap and an α limiter that may push to recover (both in `systems/FBFlightControl`), plus the pilot hooks `BfmSearchRollCap` (0.20) and `BfmRollRateMaxDegS` (60 vs the F-16's 90) that keep the K=201 roll from a limit cycle. `mig29-bfm.fbm` now flies a full BFM run with no KO, acquires and locks a trail defender (lock_s 203, ctrl_s 5.3), and `duel-merge` survives (exit 2 → 3). F-16 byte-identical; `mig29-full` touchdown moved 143.4 → 143.7 kt (the correct `PitchStickMax` now binding in the flare). (a) ACQUISITION — **still open, and now the sole merge blocker.** The N019's close-combat modes are azimuth pencils and RAD does not auto-lock; from a stable TRAIL (`mig29-bfm`) the search-roll cap lets the platform settle and the pilot designate (lock_s 203), but in a nose-on high-closure MERGE the defensive turning fight keeps the target out of the ±30°/±6° RAD bar and the MiG never acquires (`duel-merge` lock_s 0) — so its R-73/GSh-301 thesis stays untested | an acquisition behaviour for the turning merge (RAD bar management or an ACM auto-lock path) |
