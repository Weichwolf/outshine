...
The MiG-29 counterpart to [`doc/modules/f16/`](../f16/INDEX.md). Purpose: enough distilled, cited system
...
> page everywhere — the majority convention, and the one `doc/modules/f16/` already follows.
>
...
form as `doc/modules/f16/` and `doc/`:

| Section | Meaning here |
|---|---|
| **`## Spec`** | what the real MiG-29A 9-12 documentably does — the bulk of every file, including the variant notes |
| **`## State`** | what FlightBox implements. Today that is **"nothing built"** in every single file: the airframe exists only as the spec-first contract [`module.md`](module.md). Each State section names which roadmap stage (R3 / R6 / R7 / R8) will consume which part of the file |
...
Since the Phase-3 mirror rebuild this directory holds BOTH sides of the MiG-29: the reference base
below (the **real** 9-12) and, in the same directory, [`module.md`](module.md) — the spec-first module
contract for FlightBox. **Nothing of it is built**; every `## State` section here says so and links
there.
...
|---|---|---|---|
| [module.md](module.md) | **the FlightBox MiG-29 module contract** — not a reference file: spec only, nothing built | — | the roadmap R6 stages |
