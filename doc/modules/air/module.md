...
**Status: BUILT 2026-07-28, and every deck row is `ALPHA`.** The class, the eighteen rows, the two
motion laws, the tier gate, the early-warning boundary and both attribution instruments exist and are
measured; NOT ONE generated deck is inside its anchor bands, so no row may answer a campaign question
yet. `## State` carries the numbers. It is the third
...
| **A13** | **NO CATALOGUE ROW CAN EMPLOY A WEAPON AT ALL — found by campaign O5, 2026-07-29, and it invalidates the T4 line of §Spec 5 until it is closed** | `FBAirModule` composes **no fire control**, so `FBState::FireControl` is never written for any of the eighteen rows. All three of `FBPilot`'s employment gates read it (`InterceptCommands`' `inParams` needs `fc.DlzValid && fc.InZone`, `BfmMissileShot` needs the same, the gun needs `fc.GunTolDeg`), so every one of them stays shut. **MEASURED:** an `f15c` with 4 × AIM-120 and `set brief_master_arm arm` designates at t = 8.6 s at 18.64 nm, holds `eng_locked = 1` from t = 12.1 to t = 40.1 s while closing 18.6 → 8.8 nm, and never posts a `WeaponRelease`; its `telemetry.csv` carries no `fc_*` column. `air-bomber-intercept.fbm`'s header records the symptom and attributes it to the MiG-23's radar; the cause is one layer lower and is the same on every row. **Consequence for the campaigns: an `ACCEPTED` row is accepted as a FLIGHT MODEL, and `flight-model-recipe.md`'s promotion gate measures nothing else.** O5 wanted the F-15C as an escort — the aircraft that historically shot down every MiG in both of its anchors — and flew F-16s instead ([`../../campaigns/o5-airfield-defence.md`](../../campaigns/o5-airfield-defence.md) §State) |
