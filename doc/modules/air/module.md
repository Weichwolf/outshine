...
**Status: BUILT 2026-07-28; ARMED 2026-07-29.** The class, the eighteen rows, the two motion laws, the
tier gate, the early-warning boundary and both attribution instruments exist and are measured. Four deck
rows are inside their anchor bands and may answer a campaign question; six are `ALPHA`. Since 2026-07-29
a row can also **employ what it declares** — §Spec 12's fire control, ten rows of eighteen, first kill
measured (A13 closed) — with two named limits: the two pure-cannon rows cannot RANGE a gun (A14) and no
row can yet FIGHT with one (A15). `## State` carries the numbers. It is the third
...
| A row can **employ what it declares**, through the smallest fire control that allows it | §Spec 12. `modules/air/FBAirFireControl` is composed **iff** `FBAircraftSpec::CanEmploy()` — tier ≥ T2 **and** (a station **or** a gun). Three products, each with a named reader: the launch zone (`FBStoresSystem`'s release interlock + the pilot's shot gate), the target estimate (the midcourse uplink, i.e. **the binding**), the gun solution (`BfmGunfire`). Acceptance: a T0 row's `blk_firecontrol` stays 0 for a whole run while a T4 row's reads 1 |
| **Whether a round binds its shooter is a LOADOUT fact, not an airframe fact** | `FBAirPilot::SupportInhibitsDefend()` is derived from `FBSeekerHandoverS` of the round **actually in the air**, latched at the shot. The same F-15C answers it differently with AIM-7 and with AIM-120, and `air-eagle-sparrow.fbm` / `air-eagle-amraam.fbm` are the same geometry with only the four `store` lines changed |
...
modules/air/FBAirModule                : FBModule            (rank 8)
  ├─ FBAirFireControl                         (rank 8)   §Spec 12, and NULL on a row that declares no weapon
...
| Channel | Content |
|---|---|
| Telemetry, own source `air` | `air_tier`, `air_state` (the pilot phase ordinal, incl. `Orbit`/`Drag`), `air_cue_age_s` (the two staleness terms summed, −1 when uncued), `air_cues`, `air_drag_s`, and — since 2026-07-29 — the fire control's **live** answer: `fc_dlz`, `fc_in_zone`, `fc_rng_nm`, `fc_rtr_nm`, `fc_raero_nm`, `fc_tta_s` (−1 = this round never lets go), `fc_gun_err_deg`, `fc_gun_funnel`, `fc_bound`. **A13 was found late because there was no column to find it in:** `eng_*` carries the SHOT's own snapshot and therefore says nothing about an aircraft that never shot |
...
### 12. The fire control — the minimum with which a declared weapon can leave the aeroplane

**The question A13 forced, 2026-07-29.** Eighteen rows flew and not one could shoot: `FBAirModule`
composed no fire control, `FBState::FireControl` was never written, and all three of `FBPilot`'s
employment gates read that block. The F-16 answers the same question with `FBF16FireControl` — four
products, a delivery-mode state, an air-to-ground integration and a steerpoint ranging line. **A
catalogue cell needs the part of that a campaign's verdict hangs on, and the file has to say which part
it dropped and why**, or the coarseness is an accident instead of a design.

| Kept | Why it decides |
|---|---|
| The **launch zone** for the round on the selected rail (`weapons/FBLaunchZone`, unchanged) | it is the release interlock (`FBStoresSystem::Release` refuses without `DlzValid`/`InZone`) **and** the pilot's shot gate (`inParams`). Without it a row with four AIM-120 holds a lock for 28 s and never presses — the measured symptom |
| The **target estimate** a round is programmed with | it is the midcourse uplink, and therefore **the binding**: `FBStoresSystem` stops radiating the moment the estimate goes invalid, so a semi-active round loses its guidance when its shooter loses the track. Measured both ways (`air-eagle-sparrow.fbm` holds it 39.2 s to impact; `air-bomber-intercept.fbm` loses it with 9.9 s to run and the round arrives at nothing) |
| The **gun solution** off the same track | ten of the eighteen rows carry a cannon, and `BfmGunfire`'s trigger gate is `GunTolDeg`/`GunInFunnel` |

| **Dropped, deliberately** | Why it decides nothing here |
|---|---|
| `SolveGroundAttack` — the CCIP/CCRP integration, `FBReleaseSolution`, `FBDeliveryMode` | **no catalogue tier accepts `attack`** (§Spec 5 lists `route`/`orbit`/`bfm`/`intercept`/`formation`), and no catalogue radar can see the ground (A7). It would be a solution with no reader. The three ground-attack rows bomb a briefed steerpoint exactly as A7 already states |
| The `'B'` steerpoint ranging (`SteerSlantNm`, `RangeProvider`) | it is a **display** product — a number a HUD prints beside a range. §Spec 2 already refuses cockpits and displays to a catalogue cell, so this box would compute a figure nobody reads. Its absence also means the fire control does **not** gate on the Nav block, which the F-16's does |
| A **second** target picture (the F-16 keeps its own `FBBfmTrack` beside the pilot's, and so does this box — but nothing else) | no ACM box set, no raid assessment, no TWS, no mode table: §Spec 2's "radar modes beyond search and STT" line, applied to the computer as well as to the antenna |
| A per-row **funnel geometry** from sources | none exists for any catalogue gun. Instead the funnel is a **time-of-flight window**: the F-16's published 600 ft / 3 000 ft divided by the M61A1's own 1 030 m/s give **0.178 s / 0.888 s** [DERIVED], and one window then serves six guns. Cross-check against an independent source: 0.888 s at the GSh-301's 860 m/s is 764 m, against that gun's documented **800 m** effective air-target limit |
| A per-row **assumed target span** from sources | the F-16 assumes a like-type target and uses its own wingspan; a catalogue row does the same with **its own published span**, which is the one sensor input the catalogue can source for every row (§Knowledge 3) |

**Who gets one, and it is a member rather than a branch:** `FBAircraftSpec::CanEmploy()` = tier ≥ T2 **and**
(`Stations > 0` **or** a gun). Ten deck rows in, all eight movers out. A tanker therefore has no
fire-control computer at all, which is the tier statement of §Spec 5 made checkable in one column.
...
new slot. Ten JSBSim decks and seven missile decks under `sim/assets/aircraft/`, all GENERATED by
...
| **The catalogue can fight — A13 CLOSED 2026-07-29** | `modules/air/FBAirFireControl` (§Spec 12), 10 rows of 18, and the measurements are the missions below. **First catalogue kill:** `air-eagle-amraam.fbm`, an `f15c` designates at 19.40 nm, releases an AIM-120 at t = 216.3 s / 25.35 km, `SEEKER_ACTIVE` at t = 225.0 s, **`DETONATION` at t = 244.2 s with a 0.905 m miss** and `damage KILL unit=bear`. Exit 1 |
| **Binding is a loadout fact, measured on ONE geometry with ONE line changed** | `air-eagle-sparrow.fbm` against `air-eagle-amraam.fbm`, four `store` lines apart: `ttaS` **−1 vs +8.25 s**, shooter tied to the target **39.2 s (to impact) vs 8.6 s**, `fc_bound` **394 ticks vs 0**, both killing (miss 1.72 m / 0.905 m). The negative half — what a BREAK in the guidance costs — is `air-bomber-intercept.fbm`: `sms RELEASE` r24r at t = 195.3 s / 20.78 km, `missile ILLUMINATION_LOST` at t = 212.3 s with 9.9 s of a 26.75 s flight still to run, **no detonation** |
| A T0 row gets **no fire control at all** | `air-tanker-orbit.fbm`: `blk_firecontrol` = 0 over all **1 965** rows and every `fc_*` column at its "nothing here" value, against 1 (Valid) on the F-15C of `air-eagle-amraam.fbm` |
| The cannon fires | `air-fishbed-guns.fbm`: `pilot GUN_TRACK` at t = 4.5 s (464 m, aim error 8.72°, span 15.4 mr against an 11.27 mr funnel bottom), two `gun TRIGGER`, **four `gun HIT`**, `damage SYSTEM structure degraded` on the An-26 at t = 8.8 s. `fc_dlz` stays 0 for the whole run because the rails are empty — the row's armament decides which product exists |
| **The briefed GCI call was entered wrong, and it cost an intercept** | `set brief_gci <bearing>` is a **TRUE** bearing; `FBAirModule` posted it as a BODY-RELATIVE azimuth, so `set brief_gci 90` parked a MiG-23's ±30° antenna 90° off its own nose and nothing ever re-centred it (the intercept machine commands elevation only). Now converted on the first flown tick against the aircraft's own heading and altimeter, as the MiG-29's has always been. **Measured: zero radar contacts before, first contact at 27.37 nm after, on an unchanged mission file** |
...
| B4 | **PARTLY RESOLVED 2026-07-29, and the remainder is measured rather than argued.** Recipe **step 8** has now run on the four rows whose decks passed §7.1 and written their measured plants into the catalogue (`f15c` 0.680712 / 176.220, `mig21` 0.724278 / 318.528, `mirf1` 0.745189 / 273.051, `f5e` 0.757465 / 322.722 — the harness's own ROLL-A / ROLL-K anchors), so `set task bfm` is accepted on those four and the cannon is reachable. The **six ALPHA rows keep a zero plant and stay refused**, `air-tier-refused.fbm` included and byte-identical. What close combat then does on a generated deck is A15 | 4 of 10 admitted |
...
|---|---|---|
| the module registry walking a catalogue | `FBStoreModuleRegistration`, `FBSiteModuleRegistration` | one class, 18 registry names |
| an optional airframe on a unit | `units/FBSimUnit` (`std::unique_ptr<FBFdm>` may be null) | every mover row |
| the generated-deck precedent | seventeen FlightBox-own models from three recipes | the proof that "N models from one recipe" is a built form |
| the anchor-measured deck precedent | `sim/assets/aircraft/mig29` + `make test-mig29` | the recipe's whole method and its bands |
| the generic pilot with virtual airframe hooks | `pilot/FBPilot` | four of five tiers, no new pilot class |
| radar volume/frame/firming, RWR, IRST, the eye | `sensors/` | every row's perception, by configuration |
| a report that is a point with an age and no identity | `FBNetReport` + `FBDatalinkSystem` | the early-warning cue |
| weapon-as-unit, three seeker kinds, chaff and flare | `weapons.md`, `modules/missile/` | every catalogue round; **no new seeker kind** |
| damage register and the failure→`Invalid` coupling | `core/` | all of §Spec 8, free |

**The two core changes this round did ask for, both one hook wide:**
`sensors/FBRadarSystem::LookDownFactor(elevAngleDeg)` (default 1.0 — every existing set unchanged) and
`sensors/FBDatalinkSystem::Block(FBState&)` (default `state.Datalink` — likewise). Plus
`FBModule::Airborne()`, default false.
...
| **A13** | **CLOSED 2026-07-29 by `modules/air/FBAirFireControl` (§Spec 12). THE MEASUREMENT STAYS, because it is what the round was for** | **WAS, found by campaign O5, 2026-07-29:** `FBAirModule` composed **no fire control**, so `FBState::FireControl` was never written for any of the eighteen rows. All three of `FBPilot`'s employment gates read it (`InterceptCommands`' `inParams` needs `fc.DlzValid && fc.InZone`, `BfmMissileShot` needs the same, the gun needs `fc.GunTolDeg`), so every one of them stayed shut. An `f15c` with 4 × AIM-120 and `set brief_master_arm arm` designated at t = 8.6 s at 18.64 nm, held `eng_locked = 1` from t = 12.1 to t = 40.1 s while closing 18.6 → 8.8 nm, and never posted a `WeaponRelease`; its `telemetry.csv` carried no `fc_*` column. `air-bomber-intercept.fbm`'s header recorded the symptom and attributed it to the MiG-23's radar; **the cause was one layer lower and the same on every row.** **IS:** ten of eighteen rows compose a fire control, `air-eagle-amraam.fbm` is the first catalogue kill (AIM-120, 0.905 m miss, `damage KILL`), and the nine `fc_*` columns exist so the next such hole is visible in a column instead of in an absence. **What has NOT changed: an `ACCEPTED` row is still accepted as a FLIGHT MODEL** — `flight-model-recipe.md`'s promotion gate measures anchors and now, through step 8, a roll plant; it measures nothing about weapons |
| **A14** | **THE TWO PURE-CANNON ROWS STILL CANNOT AIM, and the reason is ranging rather than fire control** | `mig17` and `su7` are the rows whose ONLY weapon is a gun (`../ground/catalogue.md`'s form: 2 × NR-23 + N-37, 2 × NR-30) and both carry `SearchRangeM = 0` by design. A gun solution needs a **range**; `FBVisualContact` publishes a bearing, an elevation and an angular SIZE and no range at all, so `FBAirFireControl::SolveGun` has no track to stand on and `fc_gun_err_deg` stays −1 for their whole life. `f5e` and `su22` are in the same position for their IR rounds, and `BfmMissileShot` additionally requires `state.Radar.LockIndex >= 0`, which a row with no radar can never produce. **The fix is one mechanism and it is named rather than built: a gyro gunsight ranges on a DIALLED-IN wingspan** — subtense over an assumed extent, the identical assumption the funnel already makes — and it is ~40 lines in this box. It is not in this round because the pilot's own picture (`pilot/FBBfmTrack`) is radar-fed too, so a visually-ranged gun solution would have no pursuit law to fly it: **both halves have to be paid for at once or neither is worth anything** |
| **A15** | **CLOSE COMBAT ON A GENERATED DECK CANNOT CLOSE, AND DOES NOT RECOVER — new, and it is what step 8 exposed** | With `mig21`'s own measured plant the `Bfm` phase now runs, and it was measured on `air-fishbed-guns.fbm` and its variants. From **3.0 nm astern of a 240 kt An-26 it took 76 s to reach 0.9 nm** and then overshot; from a 760 m bounce it tracks, fires two bursts and scores four hits, and after `pilot GUN_BREAK` at 66 m it **never re-attacks** — it descends from 5 000 m and hits the ground at t = 147.9 s (`CRASH`, impact 424 m ASL), with `BfmFloorFt` 2 000 ft failing to hold it. So a catalogue row can *shoot* its gun and cannot *fight* with it. This is a pilot-law-on-a-raw-airframe defect with its own cause, it is separate from the fire control, and the honest consequence is that **no campaign may score a catalogue gun engagement yet** |
