...
| Doctrine is **mission data**, capability is **catalogue data** | `set emcon`, `set alert`, `set rounds`, `set engage_max_m`, `set reaction_s`, `set scoot_s` — six keys, each `[SET]` with a reason. Everything else about the site is said by its module name |
| Emission policy is enforced by a **sensor**, never by knowledge | `emcon hold` means the set is unpowered until the site's own **passive** receiver hears an airborne emitter. A timer or a range trigger would be the site knowing something it never measured |
...
| `Dark` | `set emcon hold` and no cue | nothing | the ESM hears an airborne emitter (bearing only) |
...
| G2 | **Suppressed ≠ destroyed** | `CombatEffective()` = `Structure` alone on a site (§Spec 8). A battery with a dead radar and dead rails still scores as effective, and SEAD is exactly that outcome. The §Spec 8 layout choice mitigates and does not solve; expressing it needs an objective kind, i.e. a `C12`-class round of its own with its own conservation argument ([`../../missions/verdict.md`](../../missions/verdict.md)) |
| G3 | **No jamming of any kind** (`C13`) | the site has no ECCM and nothing to resist. The answer to "what happens when it is jammed" is *nothing happens*, and O1's decisive mechanism stays absent |
...
| G9 | **One channel, no fire distribution** | a site engages the first firm track that passes the envelope test. There is no threat evaluation, no priority by closure or by type, and no coordination between two sites |
