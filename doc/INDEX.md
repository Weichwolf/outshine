...
| [campaigns/](campaigns/INDEX.md) | the **ten scenario specifications** — five flown by the F-16, five by the MiG-29, ten missions each, every anchor cited and tiered; plus the aggregated cast list, the capability gaps ordered by blocking degree, the identification task as an anti-cheat test and Bekaa as a measurable yardstick. **Eight of ten BUILT AND FLOWN** (O4, O1, O5, O2, W5, W3, W4, O3 — eighty `.fbm` and eight `.fbc`, every one with both determinism criteria measured; only W1 and W2 remain spec-only) |
