...
deliberately *not* written like `doc/f16/flight-model.md`, because that file **describes a model
...
**Gliederungsvorlage:** `doc/f16/flight-model.md` **§11 — Übergabe-Checkliste**, row by row. Row
...
`doc/mig29/weapons.md` §6 because it belongs in both files):
...
1. Take the pinned JSBSim F-16's tensor as the reference fighter `[XML, doc/f16/flight-model.md §3]`:
...
| **PTB-1500 centreline** | **1,500 L (T3: 1,500–1,520)**, ferry only; **9-12 has no wet wing pylons** (`doc/mig29/weapons.md` §2.2) | — | — |
...
| **Structural failure load** | **JSBSim has none** (same as the F-16, `doc/f16/flight-model.md` §4.3) | — | the physics verdict stays with `core/FBFlightMonitor`; **do not invent a break load in the XML** |
...
|---|---|---|
| Idle → max AB | **≈ 4 s** (T3) | JSBSim `FGTurbine` uses `<bleed>`/N2 seek constants; **the F-16 deck's default spool-down is 3× faster than spool-up, which `doc/f16/flight-model.md` §12.3 flags as physically backwards** — set both explicitly here rather than inheriting the default `[SET]` |
...
2. Take the **shape** of the (Mach, altitude) surfaces from a comparable low-bypass afterburning
   ...
   (`doc/f16/flight-model.md` §5.2) is the obvious candidate `[ANALOGY]`. Bypass ratios are close
...
5. **`[XML]` clamp behaviour matters.** `doc/f16/flight-model.md` §12.3 flags the F-16 deck's
...
| `Cmadot` | **SET** | note `doc/f16/flight-model.md` §12.1 records that the F-16 deck **omits** `Cmadot` while the Mk-82 deck has one. Include it here or state its absence |
...
4. **Where the two DCS manuals disagree**, both figures are recorded and the model follows the one
   named in §8; the other is listed as a known miss. (`doc/mig29/weapons.md` §7.1 lists the
...
- **JSBSim's own trim behaviour** for a twin-engine, statically-stable airframe. The F-16 model's
  `"udot doesn't appear to be trimmable"` failure above 580 KCAS (`doc/f16/flight-model.md` §12.4)
...
- Companion file: `doc/mig29/weapons.md`. Structure template: `doc/f16/flight-model.md` §11.
