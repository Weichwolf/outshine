...
`doc/mig29/`, and honestly so: **the DCS module does not implement the guidance datalink at all**, and
...
> fighter a vector**. That is the exact inverse of `doc/f16/datalink-iff.md`, where the aircraft is a peer
...
`FBCommandBus`, and exactly the kind of thing `doc/f16/controls-commands.md` §5 calls a *head-down
...
(`doc/f16/datalink-iff.md`) — the *only* cooperative identity channel is the radar interrogator, and
