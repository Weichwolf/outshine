...
`systems/FBStoresSystem`: the F-16 semantics ("release one store from a named station") are the
...
**Rebuild note.** `systems/FBEngagement`'s `Support` state currently holds the lock "until the
...
| Quantity | Formula | GSh-301 | M61A1 (from `doc/f16/weapons.md`) |
|---|---|---|---|
...
of the F-16's EEGS Level II/Level V relationship in `doc/f16/weapons.md` §2.5, where the funnel is
...
- Companion file: `doc/mig29/flight-model-spec.md` (airframe, engine, envelope anchors).
- Format template: `doc/f16/weapons.md`.
