...
| **`C1`** | **BUILT 2026-07-28, and both asks landed exactly as written.** (a) `FBUnitSignature::Radar` is an array of `kMaxEmitterBeams = 2`; `FBRwrSystem::Run` loops the beams, picks the strongest EMISSION (mode ordinal, then received power) and classifies ONCE with the final arrival bearing — deliberately not sorting on the classified mode, because `ClassifyMode` is a device hook with memory (the SPO-15 marks an azimuth channel) and must run once per emitter per tick. Measured: **303/303 telemetry files and 104/104 event logs of the 104 existing missions byte-identical** at `--threads 1/2/4`. (b) The two `Surface*` kinds are appended and the SPO-15 rule stays unwired. The perception boundary is unchanged at **six** files and `verify-layers` now PRINTS that number, because the criterion as written read the wrong line. The original text: **The surface emitter, specified in [`modules/ground/module.md`](modules/ground/module.md), asks this file for exactly two things** — and for nothing else, which is the point. (a) `FBUnitSignature` carries **two** emitter beams instead of one, because a battery radiates search and track simultaneously and collapsing them by precedence would delete the search→track transition for every observer except the one being tracked; aircraft write index 0 only, so every existing mission stays byte-identical by construction. (b) `FBEmitterKind` gains `SurfaceEarlyWarning`/`SurfaceFireControl`, **appended** — which is also the first discriminator gap 25's assumed-altitude priority hook could ever key on, deliberately left unwired there because the real SPO-15 rule keys on threat letters this tree does not model. `FBEmitterSignature` itself is **unchanged**, and the `RESTRICTED` list does **not** grow: the site's four detectors derive from `FBRadarSystem` (×2), `FBVisualSystem` and `FBRwrSystem`, and a derivation adds no include |
