...
`doc/` already mirrors `src/` by rule. `test/` does not exist: ten harnesses live in
`sim/src/clients/FBTest*.cpp`, i.e. under a directory that means *client applications*. Under this rule
each becomes a runner plus a declaration file beside the code it judges.
...
Nothing built. Today's fix to `FBTestAirEnvelope`'s exit code is a patch on the old shape, not this one.

## Gaps

- **Seven anchors are outside their bands right now.** Reading them one by one — model defect or band
  too tight — is owed, and it must not be settled by widening a band.
...
- The ten existing harnesses are C++ with hand-rolled verdicts; migrating them is the bulk of the work
  and each one must be re-measured, not ported on faith.
