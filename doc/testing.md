...
| `make -C sim verify-trees` | `tools/verify_trees.py`, the triad gate, **three scopes with three rules** ([`mods.md`](mods.md) §3, [`module-contract.md`](module-contract.md) §5): the engine's three trees by path congruence, each mod by doc-plus-proof, each `src/modules/<id>/` by its four fixed parts — class, registration, `module.md`, and a `**Contributes:**` line in the closed vocabulary of `src/modules/FBContribution.h`. Prints the count per scope and every orphan by scope, name, kind **and remedy**. **20 engine** (9 `MISSING`, 8 `LEAF`, 3 `EXTRA`) + **3 mod** + **2 module** (`missile`, `stores`: no `module.md` to declare in) — red, and naming why |
...
| `make -C sim test-air` | builds the harness **and** judges it. **5 anchors outside their band**, read one by one on 2026-08-05: `su27` A1, `mig23`/`su22` A3, `mig17` A5 + α. The two that closed — `mig25` −22.9 % → −0.5 %, `su7` −14.0 % → −4.4 % — closed because the INSTRUMENT was corrected, not the deck and not the band |
...
- **Five anchors are outside their bands**, down from seven, and **no band was touched** —
  `test/modules/air/envelope.json` is byte-identical across the round. The A3-clustering hypothesis (four
  of the seven on one anchor, in both directions, pointing at the instrument rather than at four
  unrelated decks) **held for two of the four and failed for the other two**, which is the useful shape
  of an answer: `mig25` and `su7` were the measurement (an unfinished run booked as a result, a decay
  read on vertical speed instead of `Ps`, and a schedule family with no Mach segment — the full list is
  [`modules/air/flight-model-recipe.md`](modules/air/flight-model-recipe.md) §4.4); `mig23` and `su22`
  survived every instrument correction and are deck statements with no sourced fix (R14). What remains:
  - `su27` A1 −6.3 % — measured NOT to be the clock (the run converges: M 2.1931 → 2.2030 over 660 s).
  - `mig17` A5 −11.6 % and α −5.9 % — `systems/FBFlightControl`, not the deck: a pure proportional
    limiter droops by `stick/k_p` at its own limit, all ten rows read −3.1 … −5.9 % on α, and the g
    branch limit-cycles between ±`PitchStickMax`. `k_p = 0.20` is a FLOWN airframe's measured value, so
    this is its own round with its own differential.
  - **A correction can make a number worse and still be a correction.** Three of the four A3 rows got
    worse at some step of the fix (`mirf1` −8.2 → −11.9, `mig23` +13.3 → +62.4), and each time the
    reason was that the instrument had stopped flattering the deck. `mig23`'s +62.4 % is what found the
    deck defect underneath (thrust tables walling at the grid edge, recipe R18).
