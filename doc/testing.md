...
| `make -C sim verify-trees` | `tools/verify_trees.py`, the triad gate, **two scopes with two rules** ([`mods.md`](mods.md) §3): the engine's three trees by path congruence, each mod by doc-plus-proof. Prints the directory count of each tree and every orphan by scope, name and kind. **20 engine orphans** (9 `MISSING`, 8 `LEAF`, 3 `EXTRA`) against 24 before the move, plus **4 mod orphans** (the four `doc/`-only titles) — red, and naming why |
