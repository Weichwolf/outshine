...
/* Every `test/render/<feature>/<case>/` that carries BOTH products. A case with neither has not been
...
std::vector<std::string> PreparedCases(size_t &unpaired) {
  ...
  const std::string root = "test/render/";
...
int main() {
  ...
  if (cases.empty()) {
    Unprepared("test/corpus/prepare.py has produced no oracle.exr/oracle.raw pair to decode");
    return Report();
  }
