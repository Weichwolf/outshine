 ...
 * mapping at all.
 *
 * WHY IT IS CHECKED HERE AND NOT ONLY BY `test/render`. A render case notices the loss only where its
 ...
 * `test/render/SurfaceIdentity.h`: the names are held against the glTF's own materials there, per
...
std::vector<std::filesystem::path> PreparedCases() {
  ...
  for (const std::filesystem::directory_entry &area :
       std::filesystem::directory_iterator("test/render", failed)) {
    if (!area.is_directory()) { continue; }
    for (const std::filesystem::directory_entry &one :
         std::filesystem::directory_iterator(area.path(), failed)) {
...
int main() {
  ...
  if (cases.empty()) {
    Unprepared("test/corpus/prepare.py has produced no provenance.json to read a mapping out of");
    return Report();
  }
