...
| `roads/CASES.md` + `roads/synthetic.py` | every case of infrastructure x terrain the planet has, in isolation: the bed builds terrain (a height function sampled at the DEM's posting and read bilinearly) x network (OSM-tagged ways), solves the map, builds the junction surfaces, checks the invariants I1-I6 and writes a picture per case | the map's profile as in band.py; a JUNCTION is the through (major) way's surface, crown and all, extended over netconvert's node polygon, and every minor leg's profile ends on it (height and grade at the node) with its section WARPED into the major's surface over its last 20 m [SET, RAS-K] -- so no step can stand along the boundary; a bridge's deck is free between abutments on the terrain; a tunnel's bore is under the terrain, and the cover short of 3 m next to a portal is the portal cutting, not a failure | measured 2026-09-05: 21 cases green -- C0 to 0, C1 to 1e-8, the DEM band held (cliff 2.8 m, terraces 0.4 m, noise 0.7 m), junction steps 0.000 m at T and X on flat, 15 % along, 15 % across and a crest; before the warp the same cases read 0.08-2.20 m (the negative control, which is what the Heidelberg picture showed); the bridge clears the water by 20 m with abutments on the terrain to 1e-5; the tunnel's cover is 1.15 m at the portal node and never negative | nothing yet: the bed grows first (bridge over road, interchange, roundabout, ramps, dual carriageway, seams, the pathologies, the welded 3D mesh), then `Path::Network` and the structure stage (board:2101) |
