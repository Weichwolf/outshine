 ...
 *
 * `test/corpus/manifest-schema.json` is the declaration and the preparer reads the same file. This
 ...
 * `test/render/<feature>/<case>/`, so the corpus root is three directories up -- the preparer that
 * reads the same file lives there. */
[[nodiscard]] inline std::string SchemaPathBesideCase(const std::string &caseDirectory) {
  return caseDirectory + "../../../corpus/manifest-schema.json";
}
