/* THE DECODER'S ONE RULE, and it is the rule the format tempts an implementer to break: the colour
 * space of a glTF image is decided by the SLOT it is used in -- base colour and emissive sRGB,
 * metallic-roughness and normal linear -- so a PNG's own `gAMA`, `sRGB` and `iCCP` chunks are
 * IGNORED. `TextureEncodingTest` renders four columns of one row precisely to catch a decoder that
 * helpfully applied a file's gamma, because that decoder transforms one slot twice and the other
 * once and the row stops being uniform.
 *
 * THE SUBJECTS ARE BUILT HERE, BYTE BY BYTE. A PNG carrying a `gAMA` chunk is not something any
 * encoder in this tree writes, and a committed one would be a painted file nobody can recompute. The
 * three below differ in exactly one chunk and in nothing else -- same IHDR, same IDAT, same pixel --
 * so a difference in what comes out has exactly one possible cause. */
#include <cstdint>
#include <string>
#include <vector>
...
/* A zlib stream of STORED deflate blocks: no compressor is needed to write a valid PNG, and one
 * written here would be a second thing that can be wrong. */
...
/* One 2x2 RGB8 image of a single colour, plus whatever colorimetric chunk the caller wants between
...
int main() {
  ...
  /* gAMA 1/2.2 against gAMA 1.0 is the largest transform a PNG can ask for and it is the one a
   * plausible decoder applies. If any byte moved, this engine would be decoding base colour through
   * two transfers and metallic-roughness through one. */
  ...
  /* The decode is the inverse of the encode on this side too, so a picture a test writes and a
   * picture a test reads cannot drift apart without one of these two moving. */
  {
  ...
  /* AN UNDECODABLE INPUT IS A REFUSAL AND NOT AN EMPTY RASTER, because an empty picture is the
   * failure the whole render suite's guard exists to catch. */
  {
    Raster nothing;
