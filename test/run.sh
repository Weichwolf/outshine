...
LayerIncludes() {
  case "$1" in
    ...
    # THE CORPUS RUNNER SEES THE DOOR AND FOUR UTILITY PATHS, AND THIS LINE IS THE GUARD. The
    # vendor cases must break when BEHAVIOUR changes and never when a file moves, so the runner is
    # not given a single engine path: no import, no render, no engine, no scene, no scenario. What
    # it does get is `include/` and the three directories holding a JSON reader, an image codec and
    # a text target -- utilities that know nothing about rendering. Widening this line back is how
    # the guarantee would be lost, so widening it is the finding.
    harness/khronos/glTF) printf '%s' "-Iinclude -Isrc/base/format -Isrc/base/io -Isrc/content/shade -Itest/harness/shared -Itest/harness/shared/render -Itest/harness/shared/corpus" ;;
