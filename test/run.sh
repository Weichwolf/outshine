...
LayerIncludes() {
  case "$(TestProfile "$1")" in
    ...
    harness/claims) printf '%s' "-Iinclude -Isrc/base/math -Isrc/base/geo -Isrc/base/format -Isrc/base/spatial -Isrc/content/shade -Isrc/world/weather -Isrc/world/sky -Isrc/base/io -Isrc/import -Isrc/render -Isrc/world/ground -Isrc/world/navigation -Isrc/generators -Isrc/generators/base -Isrc/generators/building -Isrc/generators/flora -Isrc/generators/road -Isrc/generators/terrain -Isrc/generators/water " ;;
    harness/geographiclib/geodesic) printf '%s' "-Ithird_party -Iinclude -Isrc/base/math -Isrc/base/geo -Isrc/base/format -Isrc/base/spatial -Isrc/content/shade -Isrc/world/weather -Isrc/world/sky -Isrc/base/io -Isrc/import -Isrc/render -Isrc/world/ground -Isrc/world/navigation -Isrc/generators -Isrc/generators/base -Isrc/generators/building -Isrc/generators/flora -Isrc/generators/road -Isrc/generators/terrain -Isrc/generators/water -Isrc/world/data -Itest/harness/shared" ;;
...
LayerGroups() {
  case "$(TestProfile "$1")" in
    ...
    profile/engine) printf '%s' "src/base src/base/math src/base/geo src/base/format src/base/spatial src/content/animation src/content/mesh src/content/scene src/content/shade src/content/impostor src/world/weather src/world/sky src/base/io src/import src/import/native src/render/plan src/render/draw src/render/impostor src/render/scene src/render src/render/device src/render/stages src/world/entity src/ui src/world/data src/world/ground src/world/navigation src/world/ground/tiles src/generators/base src/generators/building src/generators/flora src/generators/road src/generators/terrain src/generators/water src/generators src/base/curve src/actor/body src/host src/engine/UiSession.cpp src/engine/SubjectMaterials.cpp src/engine/RuntimeScene.cpp src/engine/Laying.cpp src/engine/GroundDiagnostics.cpp src/engine/WorldFocus.cpp src/engine/WorldPlacement.cpp src/engine/RuntimeWorldPreparation.cpp src/engine/TerrainCoverage.cpp src/engine/SceneRuntimeIntegration.cpp src/engine/FrameMeasurements.cpp src/engine/AudioScenePublication.cpp src/engine/AudioOcclusion.cpp src/engine/Framing.cpp src/engine/Declaring.cpp src/engine/Keeping.cpp src/engine/Advancing.cpp src/engine/Inspection.cpp src/engine/TilePieces.cpp src/engine/HeightSheets.cpp src/engine/HeightSheetsRefinement.cpp src/engine/streaming src/engine/Engine.cpp src/audio src/scenario/Tables.cpp src/scenario/ScenarioRead.cpp src/scenario/ReadScenarioOsm.cpp src/scenario/ScenarioWrite.cpp src/scenario/ScenarioLayer.cpp src/scenario/Views.cpp src/scenario/InputMap.cpp src/scenario/Triggers.cpp src/engine/InputPump.cpp src/engine/Assembly.cpp src/engine/SimulationState.cpp" ;;
...
GroupToolchain() {
  case "$1" in
    ...
    src/render | src/render/device | src/render/stages | src/render/Readback.cpp | src/engine/UiSession.cpp | src/engine/Laying.cpp | src/engine/GroundDiagnostics.cpp | src/engine/WorldFocus.cpp | src/engine/WorldPlacement.cpp | src/engine/RuntimeWorldPreparation.cpp | src/engine/TerrainCoverage.cpp | src/engine/SceneRuntimeIntegration.cpp | src/engine/FrameMeasurements.cpp | src/engine/AudioScenePublication.cpp | src/engine/AudioOcclusion.cpp | src/engine/Framing.cpp | src/engine/Declaring.cpp | src/engine/Keeping.cpp | src/engine/Advancing.cpp | src/engine/Inspection.cpp | src/engine/TilePieces.cpp | src/engine/HeightSheets.cpp | src/engine/HeightSheetsRefinement.cpp | src/engine/streaming | src/engine/RuntimeScene.cpp) LayerToolchain render ;;
    *) printf '%s' "$CXXSTD -fno-exceptions" ;;
  esac
}
...
if [ -n "$SUITES" ]; then
  ...
  for named in $SUITES; do
    under=""
    for candidate in $TESTS; do
      case "${candidate#test/}" in "$named"/* | "$named".cpp | "$named") under="$under $candidate" ;; esac
    done
    [ -n "$under" ] ||
