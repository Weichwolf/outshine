...
// THE GRAND CANAL -- water as a mirror and a lens, buildings standing in it.
//
...
// the water field lays geometry and nothing more: no planar reflection, no refraction through
// the surface, no waves (board:2012)
//
// WHAT I EXPECT TO SEE, written before looking.
//   Building footprints beside a flat coloured polygon where the canal is. The water will read as
//   a lake of paint: no reflection of the buildings, no refraction, no wave normal, no motion.
//   I expect this to be the least convincing of the five, and to be convincing about WHY -- the
//   geometry is right and everything that makes water water is missing.
...
int main(void) {
  ...
  watches.Sees.Stands.LatitudeDeg = 45.438;
  watches.Sees.Stands.LongitudeDeg = 12.3358;
  watches.Sees.Stands.HeightM = 8.0;
  watches.Sees.Stands.SamplesHeight = true;
  watches.Sees.Stands.BearingDeg = 30.0;
  watches.Sees.Stands.PitchDeg = -4.0;
  watches.Sees.FovDeg = 70.0;
  stands.Views.push_back(watches);

  if (!engine.declare(stands) || !engine.assemble()) {
    Unprepared((std::string("Venice needs terrain and OSM tiles and this machine has none "
                            "cached: ") +
                engine.error())
                   .c_str());
    return Report();
  ...
  for (int step = 0; step < kSteps; ++step) {
    if (!engine.advance() || !engine.render(outshine::Extent{})) {
      Unprepared((std::string("Venice did not advance: ") + engine.error()).c_str());
      return Report();
    }
  }
