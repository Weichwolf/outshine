...
int main() {
  ...
  for (size_t y=90; y<230; ++y) {
    for (size_t x=90; x<230; ++x) {
      ...
      if (frame[at+1] <= 0.1f) { worst = 1; continue; }
  ...
  CHECK(engine.renderer().render({}) && engine.renderer().readPixels(Buffer::Linear, repeat), "repeat renders");
  CHECK(frame == repeat, "resident minified image repeats exactly");
  std::filesystem::create_directories("build/native-materials");
  CHECK(engine.renderer().saveScreenshot("build/native-materials/minified.png").has_value(), "native map PNG is written");
  return Report();
}
