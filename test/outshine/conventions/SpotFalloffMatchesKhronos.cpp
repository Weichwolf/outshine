...
int main() {
  ...
  for (size_t at = 0; at < radiance.size(); ++at) {
    ...
    if (!engine.drawsInto({65, 65}) || !engine.declare(declaration) || !engine.setGeometry(geometry) ||
  ...
  CHECK_NEAR(radiance[1] / radiance[0], 0.25f, 0.002f, "ratio", "Khronos cosine midpoint: ((0.75 - 0.5) / (1 - 0.5)) squared");
  CHECK_NEAR(radiance[2] / radiance[0], 1.0f, 0.002f, "ratio", "spot axis preserves point intensity at equal distance");
  return Report();
}
