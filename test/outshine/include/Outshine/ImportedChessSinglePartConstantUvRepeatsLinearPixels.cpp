...
int main() {
  ...
  std::vector<float> constantUv(single.textureOf(*part).size(), 0.5f);
  CHECK(single.setTexture(*part, constantUv), "single part uses constant first UV");
  ...
  for (int repeat = 0; repeat < 3; ++repeat) {
    ...
    CHECK(first == repeated,
          "single native chess part with constant UV repeats every linear channel exactly");
  }
  std::filesystem::create_directories("build/native-materials");
