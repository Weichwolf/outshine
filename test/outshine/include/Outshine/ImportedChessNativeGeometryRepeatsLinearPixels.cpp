...
int main() {
  ...
  if (!accepted(engine.setRenderTarget({1280, 720})) || !accepted(engine.declare(scenario)) ||
      !accepted(engine.setGeometry(native)) || !accepted(engine.assemble()) ||
  ...
  CHECK(
      engine.renderer().saveScreenshot("build/native-materials/chess-native-first.png").has_value(),
  ...
  for (int repeat = 0; repeat < 3; ++repeat) {
    ...
    CHECK(firstDepth == repeatedDepth, "native imported chess depth repeats exactly");
    CHECK(worst <= kStableLinearDelta,
          "native imported chess filtering remains within one linear 8-bit code");
  }
  std::filesystem::create_directories("build/native-materials");
