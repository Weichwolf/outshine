...
int main() {
  ...
  scene.Motion.StepS = 0.02;
  scene.Volumes[0].When = "dwell";
  scene.Volumes[0].DwellS = 0.035;
  CHECK(engine.declare(scene) && engine.assemble(), "dwell scene uses its declared physics step");
  ...
  CHECK(fired() == 0, "one declared step remains below the dwell threshold");
  CHECK(engine.advance().has_value(), "third dwell step crosses the threshold");
  CHECK(fired() == 2, "both entities dwell after two declared physics steps");
