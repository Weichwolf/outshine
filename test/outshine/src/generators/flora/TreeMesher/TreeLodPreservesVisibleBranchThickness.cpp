...
float Rightmost(const outshine::Generators::TreeMesh &mesh) {
  float most = 0;
  for (size_t at = 0; at < mesh.BarkVerts.size();
       at += outshine::Generators::TreeMesh::kBarkFloats) {
...
int main() {
  ...
  CHECK(Rightmost(coarse) < 0.1f,
        "a long branch below one pixel in diameter has no explicit coarse mesh");
  CHECK(Rightmost(fine) > 0.29f, "moving close enough restores the full branch length");
  CHECK(Rightmost(unbounded) > 0.29f,
        "negative control: disabling the pixel threshold keeps the thin branch");
