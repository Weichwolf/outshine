...
Bytes Line(std::optional<uint64_t> id) {
...
int main() {
  ...
  const std::array features{Line(std::nullopt), Line(0), Line(kLargestId)};
  const Bytes valid = Tile(features);
  OsmVector decoded;
  ...
  CHECK(!decoded.Features()[0].SourceFeatureId && decoded.Features()[1].SourceFeatureId == 0 &&
            decoded.Features()[2].SourceFeatureId == kLargestId,
  ...
  const Bytes repeated = Tile(std::array{Line(kLargestId)});
  ...
  CHECK(first && second && otherFirst && otherSecond,
        "both tile arrival orders publish the source IDs");
  ...
  for (size_t at = 0; at < 4; ++at) {
    matching &= forward.Features()[at].SourceFeatureId == reverse.Features()[at].SourceFeatureId;
  }
  CHECK(matching && !forward.Features()[0].SourceFeatureId &&
            forward.Features()[1].SourceFeatureId == 0 &&
            forward.Features()[2].SourceFeatureId == kLargestId &&
            forward.Features()[3].SourceFeatureId == kLargestId,
        "native IDs survive canonical tile order and repeated cross-tile identity");

  Bytes duplicated = Line(7);
  ...
  const auto rejected = forward.Accept(3, 1, Tile(std::array{duplicated}));
  ...
  Bytes wrongWire = Line(7);
  wrongWire.push_back(0x0a);
  wrongWire.push_back(1);
  wrongWire.push_back(0);
  const auto refused = forward.Accept(3, 1, Tile(std::array{wrongWire}));
  ...
  CHECK(corrected && forward.Settled(3, 1) && forward.Features().size() == accepted + 1 &&
            forward.Features().back().SourceFeatureId == kLargestId,
        "corrected retry publishes the source ID once");
  return Report();
}
