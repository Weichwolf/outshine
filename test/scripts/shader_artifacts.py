...
selection, host struct layout, graphics interface linkage or backend execution.
"""
import argparse
...
def audit(root, manifest, tool, report, timeout=30, jobs=4):
    started = time.monotonic()
    ...
    summary = {'tool': resolved_tool, 'tool_sha256': tool_hash,
               'inventory': str(manifest), 'artifacts': records,
    ...
    print('shaders: renderer selector/Shape coverage, interface linkage and backend execution remain separate checks')
...
def main():
    ...
    return audit(root, args.manifest or root / 'build/shader-artifacts.txt', args.tool,
