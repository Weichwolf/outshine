...
#
# EVERY BUILD RESTATES WHAT THE LIBRARY IS. build/STATE is generated by `run.sh --state` on every
...
all:             ## compile the library entire -> build/liboutshine.a, and state it -> build/STATE
	@cd $(SELF_DIR) && sh test/run.sh --library
	@cd $(SELF_DIR) && sh test/run.sh --state > build/STATE
