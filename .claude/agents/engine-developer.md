...
**`doc/requirements.md` is the scope and the authority on what the engine must do.** `doc/todo.md` is the
current work item; `doc/bugs.md` is what exists and is wrong; both belong to the architect and you write
into `bugs.md` only when a round asks you to. `doc/` also holds the C++ Core Guidelines whole and the
index of them appended below. **`CLAUDE.md` is the vision, the four constraints, the stance and the
...
**The whole standard is in the tree** at `doc/CppCoreGuidelines.md` (514 sections), and **all 511 rule
...
a question, `doc/CppCoreGuidelines.md` is what settles it. This is an index, not the standard.
