...
- Line 52: `if (!(driven > 0.0)) { driven = braked; }` is a dead fallback. Line 73 assigns
  ...
  contacts.
- Lines 49/53: `steered` is summed and floor-defaulted but divides nothing —
  `SteeredShare` is the raw predicate (line 72). Dead arithmetic; delete it or use it.
