...
- [x] Sky irradiance as the ambient scale — `MediumSkyIrradiance` integrates the dome cosine-weighted at stand-up and `Live` feeds E/pi into the environment term; the GPU `IrradianceStage` (per-frame, for a moving sun) remains scope
