...
allocation, mapping, copies, submission and CPU retention. Subject stream
submission is tracked separately by WI 2263.
...
staging and submission fences, while `SceneResources` owns the CPU restore
copy. Neither consumer sees partial classification. A complete, validated pair
of buffers and its CPU copy becomes visible together; on error or stale
...
palette storage, fill and submit bounded byte ranges, finish the CPU restore
copy, poll the final GPU fence without waiting, then commit both bindings.
...
1. Attribute allocation, map/zero/copy, submit/fence, CPU restore copy and
   ...
   and `Laying`. Measure byte counts and longest complete frame slice. If one
...
3. Extend `GroundStoragePublishesCompleteUploads` with inputs on both sides of
   ...
   fail readback. Extend `GroundClassificationBelongsToItsWorld` for the staged
   CPU copy and atomic commit. Compare synchronous and paced output.
