...
- **Six moved and are still there**, from § I.26.9's re-organisation of the unit suite: `test/outshine/unit/core/PlanarGeodesyHoldsToItsScope.cpp`, `test/outshine/unit/data/AbsenceHandsOver.cpp`, `test/outshine/unit/data/TheAnswerNamesItsAddress.cpp`, `test/outshine/unit/data/UncoveredIsUndeclared.cpp`, `test/outshine/unit/generators/SameRegionSamePlacement.cpp`, `test/outshine/unit/generators/draw/GrownBarkIsAClosedMesh.cpp` — each now under `test/outshine/unit/`. **Eleven of the seventeen lines are one path-prefix edit**, and every one of those tests **PASSes** in this round's run, so the claim is true and only its address is wrong.
