...
```
  ProfiledRoadMesher.cpp:41      every vertex seeded with normal {0,1,0}
  ProfiledRoadMesher.cpp:63      face normals ADDED into shared vertices across creases, never
                       normalised; Geometry::setNormals(int, span<const float> unit) states
                       the contract they break -- a top vertex leaves with |n| of 1..3
  ProfiledRoadMesher.cpp:152     the y seed hand-zeroed for side quads: the defect noticed and patched
                       locally
  ProfiledRoadMesher.cpp:106,    float sorts on an angle with no tiebreak decide a junction's fan and
...
- `ProfiledRoadMesher` builds the way `BuildingMesh` does: a face's vertices are its own, the face
