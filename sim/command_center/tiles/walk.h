 ...
 * Builds the w3_D draw list; the per-pass split/over counters live here and world3d_stream_at reads
 * them. */
...
static void w3_walk(int z, long x, long y, double lat, double alt, double tx, double ty) {
  long n = 1L << z;
  if (x < 0 || y < 0 || x >= n || y >= n) return; /* off the map: a real hole, not a gap */
  ...
  for (int q = 0; q < 4; q++)
    w3_walk(z + 1, x * 2 + (q & 1), y * 2 + (q >> 1), lat, alt, ctx, cty);
}
#endif
