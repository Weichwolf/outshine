...
static void world3d_stream_at(double lat, double lon, double alt_agl) {
  ...
  if (!moved && w3_stream_done && w3_frame.sharpen == 0 && !W3_LOD_SPIN)
    return; /* climb keeps us awake */
  w3_O.tx = tx;
  w3_O.ty = ty;
  w3_O.have_tile = 1;
  ...
  /* w3_frame.nD>0 is not pedantry: "nothing is waiting" is also true of a tree that asked for
   * nothing. Without it the first pass over a cold region -- where every chunk is still in flight
   * -- sets done=1, this function returns early forever, and the world stays empty with no error
   * anywhere. That is exactly what happened: "0 chunks drawn, 0 waiting", then silence. */
  /* "Done" is the FLOOR criterion: every chunk drawable (256 present), nothing waiting. Sharpening
   * up the ramp (w3_frame.sharpen) is deliberately NOT here -- it keeps the streamer awake (the
   * early-out above) but must not hold the gate, or a moving camera forever mid-climb on a fresh
   * 2048 would never let "0 pending" latch. */
  w3_stream_done =
      w3_frame.nD > 0 && w3_frame.pending == 0 && w3_frame.split_wait == 0 && !w3_ground.dirty;
  ...
  if (moved || (w3_stream_done && !was_done) || w3_frame.sharpen != w3_last_sharpen || W3_TRACE) {
    ...
    printf("[world3d] quadtree: %d chunks drawn (budget %d), %d wanted split, %d waiting,"
           ...
           w3_texvram() / (1024 * 1024), w3_stream_done ? "" : " (waiting on fb-tiles)");
  }
}
#endif
