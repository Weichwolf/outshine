 ...
 *
 * The WRITER is the tile worker (chunkmesh_ecef.h -> w3_chunk_build_ecef, off the main thread). The
 * READER is the draw call on the main thread (glVertexAttribPointer, via W3_VTX_STRIDE/W3_VTX_OFF in
 ...
 * sides must agree on lives here, alone; the build that only the worker runs stays in chunkmesh_ecef.h.
 *
...
static inline void w3_chunk_free(w3_chunk *c){ if(!c) return; free(c->verts); c->verts=0; c->nverts=0; c->err=0.f; }
