 ...
 * paints characters from its tile ROM. This replaces the old vector-line font (many GL_LINE
 * segments per lit pixel -> a per-frame vertex explosion), so the per-frame HUD geometry is small
 * and bounded (visible chars x 4 verts), which is what kills the tape-label flicker at its root.
 *
...
static float mx_v[32768];
static int mx_vN;
static struct {
  GLuint prog, tex, vbo;
  GLint pos, uv, col, scale;
} mx_gl;
...
static void mx_vert(float x, float y, float u, float vv, float r, float g, float b) {
  if (mx_vN > 32761) return;
  mx_v[mx_vN++] = x;
  mx_v[mx_vN++] = y;
  mx_v[mx_vN++] = u;
  mx_v[mx_vN++] = vv;
...
/* Blit one glyph: quad at (x,y) size qs px, UV over glyph `gi`'s 8x8 tile (inset 0.05 texel so
 * NEAREST never lands on a cell boundary and picks a neighbour). */
static void mx_glyph(float x, float y, float qs, int gi, float r, float g, float b) {
  float u0 = ((float)gi * MX_TILE + 0.05f) / MX_ATLAS_W,
        u1 = ((float)gi * MX_TILE + MX_TILE - 0.05f) / MX_ATLAS_W;
  float v0 = 0.05f / MX_TILE, v1 = (MX_TILE - 0.05f) / MX_TILE, x1 = x + qs, y1 = y + qs;
  mx_vert(x, y, u0, v0, r, g, b);
  mx_vert(x1, y, u1, v0, r, g, b);
  mx_vert(x1, y1, u1, v1, r, g, b);
  mx_vert(x, y, u0, v0, r, g, b);
  mx_vert(x1, y1, u1, v1, r, g, b);
  mx_vert(x, y1, u0, v1, r, g, b);
}
...
static void mx_text(float x, float y, float s, float r, float g, float b, const char *t) {
  ...
  for (; *t; t++) {
    char u = *t;
    if (u >= 'a' && u <= 'z') u -= 32;
    const char *p = strchr(MX_CS, u);
    int gi = p ? (int)(p - MX_CS) : 0;
    if (gi > 0) mx_glyph(x, y, qs, gi, r, g, b);
    x += adv;
  }
}
...
static const char *MX_VS = "attribute vec2 aPos; attribute vec2 aUV; attribute vec3 aCol; uniform "
                           "vec2 uScale; varying vec2 vUV; varying vec3 vCol;"
                           "void main(){ gl_Position=vec4(aPos.x*uScale.x-1.0, "
                           "1.0-aPos.y*uScale.y, 0.0,1.0); vUV=aUV; vCol=aCol; }";
...
static void mx_init(void) {
  ...
  for (int gi = 0; gi < MX_NGLYPH; gi++)
    for (int row = 0; row < MX_TILE; row++) {
      unsigned char m = MX_FONT[gi][row];
      for (int c = 0; c < MX_TILE; c++)
        a[row * MX_ATLAS_W + gi * MX_TILE + c] = (m & (0x80 >> c)) ? 255 : 0;
    }
  glGenTextures(1, &mx_gl.tex);
  glBindTexture(GL_TEXTURE_2D, mx_gl.tex);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, MX_ATLAS_W, MX_TILE, 0, GL_LUMINANCE,
               GL_UNSIGNED_BYTE, a);
  ...
  mx_gl.pos = glGetAttribLocation(mx_gl.prog, "aPos");
  mx_gl.uv = glGetAttribLocation(mx_gl.prog, "aUV");
  mx_gl.col = glGetAttribLocation(mx_gl.prog, "aCol");
  mx_gl.scale = glGetUniformLocation(mx_gl.prog, "uScale");
  glUseProgram(mx_gl.prog);
  glUniform1i(glGetUniformLocation(mx_gl.prog, "uAtlas"),
              0); /* sampler on unit 0, program must be bound */
...
static void mx_render(int W, int H) {
  if (mx_vN <= 0) return;
  glUseProgram(mx_gl.prog);
  glUniform2f(mx_gl.scale, 2.0f / W, 2.0f / H);
  glActiveTexture(GL_TEXTURE0);
  glBindTexture(GL_TEXTURE_2D, mx_gl.tex);
  glBindBuffer(GL_ARRAY_BUFFER, mx_gl.vbo);
  glBufferData(GL_ARRAY_BUFFER, mx_vN * 4, mx_v, GL_DYNAMIC_DRAW);
  glEnableVertexAttribArray(mx_gl.pos);
  glEnableVertexAttribArray(mx_gl.uv);
  glEnableVertexAttribArray(mx_gl.col);
