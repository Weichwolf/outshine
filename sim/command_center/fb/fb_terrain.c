...
/* stb_image lives in osmmesh's terrain.c (STB_IMAGE_IMPLEMENTATION, non-static, shared across the
 * link — see the note there). Declare, don't re-implement: two implementations would collide. */
extern unsigned char *stbi_load_from_memory(const unsigned char *buffer, int len, int *x, int *y,
