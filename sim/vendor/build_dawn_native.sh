#!/bin/bash
# Build native Dawn (the C/C++ WebGPU implementation) for the headless CLI renderer
# (fb/gpu_native.cpp). Pin matches vendor/emdawnwebgpu_pkg/VERSION.txt so the WASM and native
# builds speak the same webgpu.h — write the shader/pipeline code once, link it twice.
# Clone is fetched fresh (not vendored in git, see .gitignore); rerun after `rm -rf vendor/dawn`
# to update the pin.
set -euo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")"

TAG=0bc38adde72b79013536f8ce354b639ae19ae195   # Dawn v20260720.160313 — vendor/emdawnwebgpu_pkg/VERSION.txt
...
# webgpu/webgpu_cpp.h (same generator as the WASM emdawnwebgpu package -> identical C++ API).
