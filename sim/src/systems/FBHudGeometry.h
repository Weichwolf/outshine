 ...
 * Two primitive kinds matching the two pipelines: Strokes (x,y,d,hw,r,g,b,a) and Glyphs
 * (x,y,u,v,r,g,b), six vertices each. Vectors, not fixed arrays: Reset() keeps the capacity, so after
 * the first frame the bounded, deterministic symbology reallocates nothing.
...
/* The bounds FBHudStage sizes its GPU buffers to (the stroke count is the one the 7-float vertex
 * carried, re-expressed for the 8-float one — same symbology capacity, not a new budget). */
inline constexpr size_t kHudStrokeFloatsPerVtx = 8;
inline constexpr size_t kHudMaxStrokeFloats = 168480;   /* 3510 quads, as before */
...
public:
  ...
  /* A FILLED rectangle at explicit opacity — the MFD bays' veil over the out-the-window picture.
   * Clipped by intersection, not by cutting a centreline. */
  void Fill(float x0, float y0, float x1, float y1, float r, float g, float b, float a);
