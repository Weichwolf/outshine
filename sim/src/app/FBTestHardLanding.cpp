 ...
 * and drives ONLY the generic simulated control surface (fcs cmd-norm via fb_jsbsim_set_controls, gear
 * via fb_jsbsim_set_aux) — no FBPilot, no FBAutopilot, nothing that writes FDM state directly (the same
...
int main() {
  ...
  fb_fdm_state st{};
  fb_jsbsim_step(&st);   /* prime st */

  const double dt = 0.01;   /* the adapter's own fixed bridge step (jsbsim_adapter.cpp SIM_STEP_S) */
  ...
  while (simT < timeoutS) {
    ...
    fb_jsbsim_set_controls(0.0, 0.0, 0.0, 0.15);
    fb_jsbsim_set_aux(1.0, -1.0, -1.0);   /* gear down; flap/speedbrake left at the model default */
