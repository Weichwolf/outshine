 ...
 * so neither App duplicates JSBSim IC / FBPilot-arming / FBFlightPlan mechanics. Header-only: both
 * entry points already link FBF16Module.cpp/FBMissionFile.cpp, so no new Makefile translation unit is
 * needed for this. */
...
/* Ground-spawns `mission`'s F-16 on its runway threshold: JSBSim IC sat-on-gear at `groundAsl`
 * (hoff_m=-1, see the adapter banner), FBAutopilot neutral Manual (idle stick/throttle — the F-16's
 * own FLCS holds wings-level) so FBPilot::Preflight's brief hold has something stable to sit in, gear
 * down + both wheel brakes set, the mission's FlightPlan/Runway wired onto the module, FBPilot armed
 * at Preflight. `aircraftPath` differs per link target (native: "vendor/jsbsim/aircraft", WASM: the
 * embedded FS path "/jsbsim/aircraft"). Returns false (JSBSim init failed) without touching f16/st. */
inline bool FBMissionGroundSpawn(const char *aircraftPath, const FBMission &mission, double groundAsl,
                                 FBF16Module &f16, fb_fdm_state &st) {
  const FBRunway &rwy = mission.Runway;
  if (fb_jsbsim_init(aircraftPath, "f16", rwy.ThresholdLatDeg, rwy.ThresholdLonDeg, groundAsl, -1.0,
                     0.0, rwy.TrueHeadingDeg, 0) != 0)
    return false;
  ...
  f16.PilotSystem().SetPhase(FBPilot::Phase::Preflight);
  st = fb_fdm_state{};
