...
ArtChannel ChannelOf(const std::string &reads) {
  if (reads == "fcs/left-aileron-pos-rad") return ArtChannel::AileronL;
  if (reads == "fcs/right-aileron-pos-rad") return ArtChannel::AileronR;
  if (reads == "fcs/dht-left-pos-rad") return ArtChannel::ElevonL;
  if (reads == "fcs/dht-right-pos-rad") return ArtChannel::ElevonR;
  if (reads == "fcs/rudder-pos-rad") return ArtChannel::Rudder;
  if (reads == "fcs/lef-pos-deg") return ArtChannel::Lef;
  if (reads == "fcs/speedbrake-pos-deg") return ArtChannel::Speedbrake;
  if (reads == "gear/gear-pos-norm") return ArtChannel::Gear;
  if (reads == "gear/tailhook-pos-norm") return ArtChannel::Hook;
  if (reads == "fcs/canopy-pos-norm") return ArtChannel::Canopy;
  return ArtChannel::None;
}

bool ChannelIsNorm(ArtChannel c) {
  return c == ArtChannel::Gear || c == ArtChannel::Hook || c == ArtChannel::Canopy;
}
bool ChannelIsRad(ArtChannel c) {
  return c == ArtChannel::AileronL || c == ArtChannel::AileronR || c == ArtChannel::ElevonL ||
         c == ArtChannel::ElevonR || c == ArtChannel::Rudder;
}
...
float UnitModel::PartAngleDeg(const Part &p, const float art[(int)ArtChannel::Count]) {
  if (p.Ch == ArtChannel::None || p.Ch >= ArtChannel::Count) return 0.0f;
...
const UnitModel::Component *UnitModel::FindComponent(const std::string &node) const {
  for (const auto &c : Components_)
    if (c.first == node) return &c.second;
  return nullptr;
}

int UnitModel::PickLod(double rangeM) const {
...
bool UnitModel::LoadDir(const char *typeName, const char *dir) {
  ...
  if (!ReadWholeFile(path, text)) {
    Log::Error("render", "unit_model_sidecar", {{"path", path}});
    return false;
  }
  Json doc;
  if (!doc.Parse(text.c_str(), text.size())) {
    Log::Error("render", "unit_model_sidecar_parse", {{"path", path}});
    return false;
  }
  const Json::Ref root = doc.Root();

  const Json::Ref comps = root["components"];
  for (size_t i = 0; i < comps.Size(); i++) {
    const Json::Ref c = comps[i];
    Component cm;
    cm.Ch = ChannelOf(c["reads"].Str());
    if (cm.Ch == ArtChannel::None) continue;   /* `(statisch)` rows: rails and stores */
    const Json::Ref lim = c["limits_deg"];
    ...
    for (const std::string &n : ExpandNodes(c["node"].Str())) {
      ...
      cm.Inverted = cm.Ch == ArtChannel::Gear && n.compare(0, 10, "gear.door.") != 0;
      Components_.emplace_back(n, cm);
    }
  }

  const Json::Ref lods = root["lods"];
  const Json::Ref steps = root["lod_switch"]["steps"];
  int built = 0;
  ...
  Log::Info("render", "unit_model", {{"type", TypeName_}, {"lods", built},
...
bool UnitModel::BuildLod(const char *glbPath, double maxRangeM) {
  Glb glb;
  if (!glb.LoadFile(glbPath)) {
    Log::Error("render", "unit_model_glb", {{"path", glbPath}, {"why", glb.Error()}});
    return false;
  }
  ...
  for (int i = 0; i < lod.MatCount; i++) {
    const GlbMaterial &m = glb.Materials[(size_t)i];
    ...
    if (m.BaseColorTex >= 0 && !lod.TexW) {
      const GlbImage &im = glb.Images[(size_t)m.BaseColorTex];
      if (im.W > 0 && im.W == im.H) {
        lod.TexW = im.W;
        lod.TexRgba = im.Rgba;
      }
  ...
  while (!stack.empty()) {
    ...
    const GlbNode &n = glb.Nodes[(size_t)v.Node];
    ...
    if (n.Mesh >= 0 && (size_t)n.Mesh < glb.MeshFirstPrim.size()) {
      ...
      for (uint32_t pi = first; pi < first + cnt; pi++) {
        const GlbPrimitive &prim = glb.Prims[pi];
