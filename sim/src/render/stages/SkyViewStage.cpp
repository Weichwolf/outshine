...
@group(0) @binding(4) var msLUT : texture_2d<f32>;
fn getValFromTLUT(pos : vec3f, sunDir : vec3f) -> vec3f {
  return textureSampleLevel(tLUT, lsamp, tLUTuv(pos, sunDir), 0.0).rgb;
}
/* Same parametrisation as the transmittance LUT, so the same uv function serves both. */
fn getValFromMsLUT(pos : vec3f, sunDir : vec3f) -> vec3f {
  return textureSampleLevel(msLUT, lsamp, tLUTuv(pos, sunDir), 0.0).rgb;
}
fn raymarchScattering(pos0 : vec3f, rayDir : vec3f, sunDir : vec3f, steps : f32) -> vec3f {
  ...
  for (var i = 0.0; i < steps; i = i + 1.0) {
    ...
    let psiMs = getValFromMsLUT(newPos, sunDir);
    let inScattering = sv.rayleigh * (rayleighPhase * sunTransmittance + psiMs)
                     + vec3f(sv.mie) * (miePhase * sunTransmittance + psiMs);
...
fn cs(@builtin(global_invocation_id) id : vec3u) {
  ...
  let lum = raymarchScattering(atmoPos(A), rayDir, A.sunDir.xyz, 32.0);
  textureStore(svOut, vec2i(i32(id.x), i32(id.y)), vec4f(lum, 1.0));
}
)";

void SkyViewStage::Configure(const Gpu &gpu, wgpu::TextureView skyLutView, wgpu::TextureView transLutView,
  ...
  be[4].binding = 4; be[4].textureView = msLutView;
...
void SkyViewStage::EncodeCompute(const FrameContext &, wgpu::ComputePassEncoder &pass) {
