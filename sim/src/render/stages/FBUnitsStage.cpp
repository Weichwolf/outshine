 ...
 * overcast lift, same day gate, same weather haze out of FBAtmoHaze.h — because a jet lit by a second
 ...
 * that chain with its hinge angle folded in. FBUnitsStage::Encode computes them; the shader only reads. */
...
/* 3x4 affine, row-major, implicit fourth row (0,0,0,1) — the same convention FBUnitModel bakes with. */
...
bool FBUnitsStage::AddModel(const char *typeName, const char *dir) {
  auto cpu = std::make_unique<FBUnitModel>();
...
const FBUnitsStage::GpuModel *FBUnitsStage::Find(const char *type) const {
...
void FBUnitsStage::UploadModel(GpuModel &m) {
  ...
  for (int i = 0; i < m.Cpu->LodCount(); i++) {
    const FBUnitModel::Lod &src = m.Cpu->GetLod(i);
    GpuLod g;
    g.IndexCount = (uint32_t)src.Idx.size();

    wgpu::BufferDescriptor bd{};
    bd.size = src.Verts.size() * sizeof(FBUnitVertex);
...
void FBUnitsStage::Configure(const FBGpu &gpu, wgpu::Sampler samp, wgpu::Sampler lutSamp,
  ...
  vbl.arrayStride = sizeof(FBUnitVertex);
  ...
  rp.fragment = &fs;
  rp.depthStencil = &ds;
  /* The asset culls back faces on every material (f16.asset.json backface_culling) — with them drawn,
   * nine inverted bodies once hid inside the mesh. The body->ECEF basis has determinant +1, so the
   * glTF counter-clockwise winding survives into clip space unchanged. */
...
void FBUnitsStage::Encode(const FBFrameContext &ctx, wgpu::RenderPassEncoder &pass) {
  LastDraws_ = 0;
  ...
  for (int i = 0; i < Count && slot < kMaxUnits; i++) {
    ...
    const FBUnitModel::Lod &cpu = m->Cpu->GetLod(li);
    ...
    for (size_t p = 1; p < nParts; p++) {
      const FBUnitModel::Part &part = cpu.Parts[p];
      const float a = FBUnitModel::PartAngleDeg(part, d.Art) * 0.017453292519943295f;
  ...
  /* The frame block, filled exactly as FBTilesStage fills its own — the two shaders read one `U`. */
...
void FBUnitsStage::LogUnit(const FBFrameContext &ctx, const FBUnitDraw &d, const FBUnitModel::Lod &lod,
  ...
  for (const FBUnitModel::Part &part : lod.Parts) {
    const double a = FBUnitModel::PartAngleDeg(part, d.Art);
...
void FBUnitsStage::LogCast(const FBFrameContext &ctx) const {
  if (ctx.FrameNo % kUnitLogEvery != 1) return;
  FBLog::Debug("render", "units", {{"cast", Count}, {"drawn", LastDraws_},
