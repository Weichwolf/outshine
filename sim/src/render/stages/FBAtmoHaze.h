 ...
 * core/FBCloudDensity.h is the one cloud field for the picture and for a sensor.
 *
...
inline float FBHazeSigma0(float visibilityM) {
...
inline float FBHazeSigmaRayleigh(float sigma0) {
  return sigma0 < kSigmaRayleigh550 ? sigma0 : kSigmaRayleigh550;
}
inline float FBHazeSigmaAerosol(float sigma0) {
...
inline float FBHazeOpticalDepth(float sigma0, float zEffM, float distM) {
  const float z = zEffM > 0.0f ? zEffM : 0.0f;
  return (FBHazeSigmaRayleigh(sigma0) * std::exp(-z / kHazeScaleRM)
        + FBHazeSigmaAerosol(sigma0) * std::exp(-z / kHazeScaleAM)) * distM;
}
inline void FBHazeOpticalDepth(float sigma0, float zEffM, float distM, float rgb[3]) {
  const float z = zEffM > 0.0f ? zEffM : 0.0f;
  const float mol = FBHazeSigmaRayleigh(sigma0) / kSigmaRayleigh550 * std::exp(-z / kHazeScaleRM);
  const float aer = FBHazeSigmaAerosol(sigma0) * std::exp(-z / kHazeScaleAM);
  for (int i = 0; i < 3; i++) rgb[i] = (kRayleighBaseRGB[i] * 1.0e-6f * mol + aer) * distM;
}
inline float FBHazeTransmittance(float sigma0, float zEffM, float distM) {
  return std::exp(-FBHazeOpticalDepth(sigma0, zEffM, distM));
}
inline void FBHazeTransmittance(float sigma0, float zEffM, float distM, float rgb[3]) {
  FBHazeOpticalDepth(sigma0, zEffM, distM, rgb);
 ...
 * the erosion at its mean — literally FBCloudDensity's fully band-limited branch (ErodeFlat = 1), i.e.
...
inline float FBDeckSunOpticalDepth(const FBCloudDeckParams &d, float sunUp) {
  ...
  for (int i = 0; i < kNodes; i++) {
    ...
    const float dens = FBCloudShape(d.Cover, h)
                     - kCloudErodeMean * d.Erosion * FBCloudMix(kCloudErodeBase, 1.0f, h);
...
inline float FBDeckFrac(const FBCloudDeckParams &d, float altM) {
 ...
 * calibrated to be an AREA FRACTION (FBCloudCalibrate, measured by --cloudcheck), so 1 - Cover of the
...
inline float FBDeckSunTransmittance(float tau, float cover, float frac) {
  if (cover <= 0.0f) return 1.0f;
  return (1.0f - cover) + cover * std::exp(-tau * frac);
}

inline std::string FBHazeConstsWGSL(void) {
  char buf[256];
...
/* Requires FBAtmoCommon.h (`Atmo`, PI) and FBAtmoSample.h (`skyViewSample`, kSkyExposure) in scope, and
...
/* One deck as (baseM, topM, sunOpticalDepth, cover) — FBDeckSunTransmittance above, evaluated where
 ...
 * (FBSunStage, character-identical below), so haze = the same sum. The glow is gated on the EVS flag
 * exactly as FBSunStage gates it — in SVS there is no sun disc and no halo, so there must be none in
 * the haze either.
 *
...
fn hazeInscatter(svLUT : texture_2d<f32>, lsamp : sampler, A : Atmo, dir : vec3f) -> vec3f {
  ...
  let halo = (exp(-sunAng * 7.0) * 0.35 + exp(-sunAng * 1.5) * 0.12 * A.skyExtra.x) * kSkyExposure;
