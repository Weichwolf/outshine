...
void StarsStage::Init(const Gpu &gpu) {
  Device = gpu.Device;
  Queue = gpu.Queue;
...
void StarsStage::SetCatalogue(const uint8_t *hyg, int nbytes, double originLat, double originLon) {
  int n = nbytes / 6;
  Cat.clear();
  Lat = originLat;
  Lon = originLon;
...
void StarsStage::Update(double nowSec) {
  if (NStars <= 0) return;
