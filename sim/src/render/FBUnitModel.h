 ...
 * device. FBUnitsStage owns the buffers. Vertrag: doc/render/units-visual.md. */
#ifndef FBUNITMODEL_H
#define FBUNITMODEL_H
...
/* The published values a part may follow (units/FBUnit.h FBUnitArticulation), as an ORDINAL so the
 * drawing side carries no dependency on the simulation's types. FBWorld maps one to the other. */
enum class FBArtChannel : uint8_t {
  AileronL, AileronR, ElevonL, ElevonR, Rudder, Lef, Speedbrake, Gear, Hook, Canopy, Count,
  None = 255
};

constexpr int kMaxUnitParts = 32;       /* the F-16 declares 20 hinges + the static root */
constexpr int kMaxUnitMaterials = 16;   /* it declares 8 */
...
public:
  ...
  struct Part {
    std::string Node;
    FBArtChannel Ch = FBArtChannel::None;
    int Parent = 0;
  ...
  static float PartAngleDeg(const Part &part, const float art[(int)FBArtChannel::Count]);
