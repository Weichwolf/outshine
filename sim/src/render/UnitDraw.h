 ...
 * Filled by World from the BORROWED registry's PUBLISHED pose (units/Unit.h) once per frame and
 * handed to Renderer; the renderer never reaches into any unit's FDM, and there is no path back.
...
struct UnitDraw {
  ...
  float Art[(int)ArtChannel::Count] = {};
