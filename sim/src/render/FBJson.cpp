...
bool FBJson::Parse(const char *text, size_t len) {
...
bool FBJson::ParseString(uint32_t &off, uint32_t &len, bool &escaped) {
...
std::string FBJson::Decode(uint32_t off, uint32_t len, bool escaped) const {
...
FBJson::Ref FBJson::Ref::operator[](size_t i) const {
  if (!Valid()) return Ref();
  const FBJson::Node &n = Doc->Nodes_[(size_t)Node];
  if (i >= n.Count) return Ref();
  return Ref(Doc, Doc->Kids_[n.First + i]);
}

FBJson::Ref FBJson::Ref::operator[](const char *key) const {
  ...
  for (uint32_t i = 0; i < n.Count; i++) {
    ...
    const FBJson::Node &c = Doc->Nodes_[(size_t)kid];
    if (c.KeyEscaped) {
