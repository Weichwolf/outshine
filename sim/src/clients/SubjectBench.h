 ...
 * the lights are the art-director's, and a hole in the matrix is a question neither can answer.
 *
...
private:
  ...
  /* `windMs` is the DECLARED met wind at 10 m for this shot. The named lights are all shot at zero:
   * a still-life judges form, and the plant's form is what the botanist pinned. The wind row is the
   * one place it is non-zero, which is also what keeps those 45 files comparable across rounds. */
  bool Shoot(const View &v, const char *lightName, double camAzDeg, double sunAzDeg, double sunElDeg,
             double cloudCover, bool metering, double windMs);
  Fill MeasureFill(const Render::BenchCard &card, double eyeAglM, double floorRadiusM, double gridM,
