 ...
 * FBFdm::SetWindNedMs), NOT the meteorological "wind from" bearing: a wind FROM 270 blows toward the
...
public:
  virtual ~FBWeatherProvider() = default;
  ...
  virtual FBWindNed WindNedMs(double latDeg, double lonDeg, double altM) const = 0;

  virtual FBCloudLayers CloudLayers(double latDeg, double lonDeg) const = 0;
