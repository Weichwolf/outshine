...
#ifndef FBCLOUDDENSITY_H
#define FBCLOUDDENSITY_H
...
inline float FBCloudSmooth(float e0, float e1, float x) {
  float t = (x - e0) / (e1 - e0);
  if (t < 0.0f) t = 0.0f;
  if (t > 1.0f) t = 1.0f;
  return t * t * (3.0f - 2.0f * t);
}
inline float FBCloudMix(float a, float b, float t) { return a + (b - a) * t; }
...
inline uint32_t FBCloudHash2(int32_t ix, int32_t iy, uint32_t seed) {
...
inline uint32_t FBCloudHash3(int32_t ix, int32_t iy, int32_t iz, uint32_t seed) {
 ...
 * they exist for. */
inline float FBCloudFade(float t) { return t * t * t * (t * (t * 6.0f - 15.0f) + 10.0f); }

inline float FBCloudGrad2(int32_t ix, int32_t iy, uint32_t seed, float dx, float dy) {
  const uint32_t k = FBCloudHash2(ix, iy, seed) & 7u;
  float gx, gy;
  if (k < 4u) {
...
inline float FBCloudGrad3(int32_t ix, int32_t iy, int32_t iz, uint32_t seed, float dx, float dy, float dz) {
  const uint32_t k = FBCloudHash3(ix, iy, iz, seed) & 15u;   /* Perlin's improved-noise gradient fold */
  const float u = (k < 8u) ? dx : dy;
  const float v = (k < 4u) ? dy : ((k == 12u || k == 14u) ? dx : dz);
  return ((k & 1u) ? -u : u) + ((k & 2u) ? -v : v);
}

inline float FBCloudNoise2(float px, float py, uint32_t seed) {
  const float fx = std::floor(px), fy = std::floor(py);
  const int32_t ix = (int32_t)fx, iy = (int32_t)fy;
  const float dx = px - fx, dy = py - fy;
  const float ux = FBCloudFade(dx), uy = FBCloudFade(dy);
  const float n00 = FBCloudGrad2(ix, iy, seed, dx, dy);
  const float n10 = FBCloudGrad2(ix + 1, iy, seed, dx - 1.0f, dy);
  const float n01 = FBCloudGrad2(ix, iy + 1, seed, dx, dy - 1.0f);
  const float n11 = FBCloudGrad2(ix + 1, iy + 1, seed, dx - 1.0f, dy - 1.0f);
  const float n = FBCloudMix(FBCloudMix(n00, n10, ux), FBCloudMix(n01, n11, ux), uy);
  return n + 0.5f;
}
inline float FBCloudNoise3(float px, float py, float pz, uint32_t seed) {
  ...
  const float ux = FBCloudFade(dx), uy = FBCloudFade(dy), uz = FBCloudFade(dz);
  const float n000 = FBCloudGrad3(ix, iy, iz, seed, dx, dy, dz);
  const float n100 = FBCloudGrad3(ix + 1, iy, iz, seed, dx - 1.0f, dy, dz);
  const float n010 = FBCloudGrad3(ix, iy + 1, iz, seed, dx, dy - 1.0f, dz);
  const float n110 = FBCloudGrad3(ix + 1, iy + 1, iz, seed, dx - 1.0f, dy - 1.0f, dz);
  const float n001 = FBCloudGrad3(ix, iy, iz + 1, seed, dx, dy, dz - 1.0f);
  const float n101 = FBCloudGrad3(ix + 1, iy, iz + 1, seed, dx - 1.0f, dy, dz - 1.0f);
  const float n011 = FBCloudGrad3(ix, iy + 1, iz + 1, seed, dx, dy - 1.0f, dz - 1.0f);
  const float n111 = FBCloudGrad3(ix + 1, iy + 1, iz + 1, seed, dx - 1.0f, dy - 1.0f, dz - 1.0f);
  const float x00 = FBCloudMix(n000, n100, ux), x10 = FBCloudMix(n010, n110, ux);
  const float x01 = FBCloudMix(n001, n101, ux), x11 = FBCloudMix(n011, n111, ux);
  const float n = FBCloudMix(FBCloudMix(x00, x10, uy), FBCloudMix(x01, x11, uy), uz);
  return n * 0.8f + 0.5f;
}
...
inline float FBCloudFbmShear(float ax, float bx, uint32_t seed) {
  float qa = ax, qb = bx, sum = 0.0f, amp = 1.0f, norm = 0.0f;
  for (int i = 0; i < kCloudOctaves; i++) {
    float sa = qa + kCloudShear * (float)i * qb;
    sum += amp * FBCloudNoise2(sa, qb, seed + (uint32_t)i * 131u);
    norm += amp;
    amp *= kCloudGain;
...
inline void FBCloudCalibrate(FBCloudDeckParams &d) {
  float p = d.Cover;
  if (p < 0.02f) p = 0.02f;
  if (p > 0.98f) p = 0.98f;
  const float z = std::log(p / (1.0f - p)) / kCloudLogitK;   /* ~ the (1-p) quantile, negated */
  d.RemapEdge = kCloudFbmMean - kCloudFbmSigma * z;
  d.RemapWidth = kCloudFbmSigma * FBCloudMix(kCloudRemapHard, kCloudRemapSoft, d.Cover);
}
...
inline FBCloudSample FBCloudCoverage(const FBCloudDeckParams &d, float eastM, float northM) {
  FBCloudSample s{0.0f, 0.0f, 0.0f};
  ...
  a /= d.Stretch;
  if (d.Warp > 0.0f) {
    float w1 = FBCloudNoise2(a * kCloudWarpFreq + 17.3f, b * kCloudWarpFreq + 5.9f, 7u);
    float w2 = FBCloudNoise2(a * kCloudWarpFreq - 3.1f, b * kCloudWarpFreq + 23.7f, 13u);
    a += (w1 - 0.5f) * d.Warp;
    b += (w2 - 0.5f) * d.Warp * kCloudWarpCross;
  }
  const float f = FBCloudFbmShear(a, b, 3u);
  s.Coverage = FBCloudSmooth(d.RemapEdge - d.RemapWidth, d.RemapEdge + d.RemapWidth, f);
  s.A = a;
  s.B = b;
  return s;
}
...
inline float FBCloudShape(float c, float h) {
  if (c <= 0.0f) return 0.0f;
  const float top = FBCloudMix(kCloudTopMin, 1.0f, c);
  const float hn = h / top;
  if (hn >= 1.0f) return 0.0f;
  return c * FBCloudSmooth(0.0f, kCloudProfBase, hn) * FBCloudSmooth(1.0f, 1.0f - kCloudProfTop, hn);
}
...
inline float FBCloudErosionFbm(float ax, float bx, float h) {
  float qa = ax, qb = bx, qz = h, sum = 0.0f, amp = 1.0f, norm = 0.0f;
  for (int i = 0; i < kCloudErodeOct; i++) {
    sum += amp * FBCloudNoise3(qa, qb, qz, 41u + (uint32_t)i * 977u);
    norm += amp;
    amp *= kCloudGain;
...
inline float FBCloudDensity(const FBCloudDeckParams &d, float eastM, float northM, float h) {
  const FBCloudSample s = FBCloudCoverage(d, eastM, northM);
...
inline FBCloudSky FBCloudSkyFromWeather(const FBWeatherProvider &wx, double latDeg, double lonDeg,
  ...
  for (int i = 0; i < 3; i++) {
    ...
    const FBWindNed w = wx.WindNedMs(latDeg, lonDeg, sampleM);
