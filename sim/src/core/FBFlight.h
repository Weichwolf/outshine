 ...
 * FBTeam.h is: it is BOTH mission data (`flight <name> <position>`) and world-entity identity — the
 ...
 * the callsign (core/FBDatalinkTrack.h). */
 ...
 * (core/FBRadarContact.h), so it cannot tell anybody. A receiver correlates the reported point
 * against its OWN contacts and may fail to — which is the honest failure mode of a shared picture,
 * not a defect. */
struct FBFlightReport {
