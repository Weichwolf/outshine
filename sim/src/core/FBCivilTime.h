...
#ifndef FBCIVILTIME_H
#define FBCIVILTIME_H
...
constexpr int64_t FBDaysFromCivil(int64_t y, unsigned m, unsigned d) {
  y -= m <= 2;
...
constexpr void FBCivilFromDays(int64_t z, int64_t &y, unsigned &m, unsigned &d) {
  z += 719468;
...
constexpr bool FBIsLeapYear(int64_t y) { return (y % 4 == 0 && y % 100 != 0) || y % 400 == 0; }

constexpr unsigned FBDaysInMonth(int64_t y, unsigned m) {
  constexpr unsigned kLen[13] = {0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
  return (m == 2 && FBIsLeapYear(y)) ? 29u : kLen[m];
}
...
inline bool FBParseIsoUtc(const char *s, int64_t &outUnixS) {
  if (!s) return false;
  size_t n = 0;
...
inline const char *FBFormatIsoUtc(int64_t unixS, char *buf, size_t n) {
  int64_t days = unixS / 86400;
  int64_t rem = unixS - days * 86400;
  if (rem < 0) { rem += 86400; days -= 1; }
  int64_t y; unsigned m, d;
  FBCivilFromDays(days, y, m, d);
