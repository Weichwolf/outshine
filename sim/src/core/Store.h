...
enum class StoreKind : uint8_t { None = 0, Mk82, Aim120, Aim9, R73, R27r,
 ...
 *   ActiveRadar     an sensors/RadarSystem of its own: goes active at its own range, after which the
 *                   shooter is free (the AIM-120's "pitbull").
 *   Infrared        an sensors/IrstSystem: ANGLES ONLY, no range and no closure, hence pure
 *                   proportional navigation on the measured line-of-sight rate — and deceivable by a
 *                   flare exactly as a radar is by chaff.
 *   SemiActiveRadar an sensors/RadarSystem that never transmits: it sees the reflection of the
 ...
 *                   MissileGuidance's strict priority (own seeker > uplink > last known) degenerates
 ...
 *   AntiRadiation   a sensors/RwrSystem of its own: a bearing, an elevation and a received power, and
...
enum class SeekerKind : uint8_t { None = 0, ActiveRadar, Infrared, SemiActiveRadar, CommandGuided,
 ...
 * outshine has no data for: EmitterSignature carries no band letter, no PRF and no pulse width, so
 * the filter is three values over EmitterKind. `[SET]` per mission (`set arm_class`).
 * doc/air-to-ground.md §2.1. */
enum class ArTargetClass : uint8_t { AnySurface = 0, SurfaceFireControl, SurfaceEarlyWarning };

inline const char *ArTargetClassStr(ArTargetClass c) {
  switch (c) {
    case ArTargetClass::AnySurface: return "any";
    case ArTargetClass::SurfaceFireControl: return "fire_control";
    case ArTargetClass::SurfaceEarlyWarning: return "early_warning";
  }
  return "?";
}

inline bool ArTargetClassFromString(const char *s, ArTargetClass &out) {
  if (!s) return false;
  if (std::strcmp(s, "any") == 0) { out = ArTargetClass::AnySurface; return true; }
  if (std::strcmp(s, "fire_control") == 0) { out = ArTargetClass::SurfaceFireControl; return true; }
  if (std::strcmp(s, "early_warning") == 0) { out = ArTargetClass::SurfaceEarlyWarning; return true; }
  return false;
}
 ...
 * already publishes (FireControlBlock::TimeToActiveS). Three answers, one per seeker kind, and the
...
inline double SeekerHandoverS(SeekerKind k, double timeToActivationS) {
  switch (k) {
    case SeekerKind::Infrared:
    case SeekerKind::AntiRadiation: return 0.0;
    case SeekerKind::ActiveRadar: return timeToActivationS;
    case SeekerKind::SemiActiveRadar:
    case SeekerKind::SemiActiveLaser:
    case SeekerKind::CommandGuided:
    case SeekerKind::None: break;
  }
  return -1.0;
}

struct StoreSpec {
  StoreKind Kind = StoreKind::None;
  const char *Key = "";        /* mission-file / ModuleRegistry name */
  const char *FdmModel = "";
  ...
  double WarheadKg = 0.0;      /* explosive+case mass — the ONE store-side input to DamageModel */
  WeaponPerf Perf;
  ...
  SeekerKind Seeker = SeekerKind::None;
...
/* Mk-82, 500 lb GP bomb. Perf.ArmingS 2.0 [SET], WarheadKg [T3], and
...
inline constexpr StoreSpec kMk82{StoreKind::Mk82, "mk82", "mk82", 500.0, 0.366, 300.0,
                                   /*Guided*/ false, /*RequiresLock*/ false, /*FuzeRadiusM*/ 0.0,
                                   /*WarheadKg*/ 87.0,
                                   WeaponPerf{/*BoostThrustN*/ 0.0, /*BoostS*/ 0.0,
...
/* AIM-120 AMRAAM; outshine-own model.
 ...
 * Derivations: doc/core.md, Abschnitt 7.1. */
inline constexpr StoreSpec kAim120{
    StoreKind::Aim120, "aim120", "aim120", 335.0, 0.115, 120.0,
    /*Guided*/ true, /*RequiresLock*/ true, /*FuzeRadiusM*/ 10.0, /*WarheadKg*/ 20.5,
    WeaponPerf{/*BoostThrustN*/ 24020.0, /*BoostS*/ 3.0,
                 ...
                 /*ArmingS*/ 1.5},
    /*Seeker*/ SeekerKind::ActiveRadar, /*FovHalf*/ 10.0, /*GimbalHalf*/ 45.0};
...
/* AIM-9M SIDEWINDER — a western infrared round.
...
inline constexpr StoreSpec kAim9{
    StoreKind::Aim9, "aim9", "aim9", 188.0, 0.0586, 60.0,
    /*Guided*/ true, /*RequiresLock*/ true, /*FuzeRadiusM*/ 6.0, /*WarheadKg*/ 9.4,
    WeaponPerf{/*BoostThrustN*/ 9885.0, /*BoostS*/ 5.2,
                 /*SustainThrustN*/ 0.0, /*SustainS*/ 0.0,
                 ...
                 /*ArmingS*/ 1.0},
    /*Seeker*/ SeekerKind::Infrared, /*FovHalf*/ 2.5, /*GimbalHalf*/ 30.0};

/* R-73 (AA-11 ARCHER) — an eastern infrared round.
...
inline constexpr StoreSpec kR73{
    StoreKind::R73, "r73", "r73", 231.5, 0.1051, 60.0,
    /*Guided*/ true, /*RequiresLock*/ true, /*FuzeRadiusM*/ 3.5, /*WarheadKg*/ 7.4,
    WeaponPerf{/*BoostThrustN*/ 11180.0, /*BoostS*/ 6.0,
                 /*SustainThrustN*/ 0.0, /*SustainS*/ 0.0,
                 ...
                 /*ArmingS*/ 1.0},
    /*Seeker*/ SeekerKind::Infrared, /*FovHalf*/ 5.0, /*GimbalHalf*/ 75.0};

/* R-27R (AA-10A ALAMO) — a SEMI-ACTIVE medium-range round.
 ...
 * switch on, so the shooter owes it illumination all the way in (SeekerHandoverS -> -1).
...
inline constexpr StoreSpec kR27r{
    StoreKind::R27r, "r27r", "r27r", 557.8, 0.1923, 90.0,
    /*Guided*/ true, /*RequiresLock*/ true, /*FuzeRadiusM*/ 13.8, /*WarheadKg*/ 39.0,
    WeaponPerf{/*BoostThrustN*/ 35900.0, /*BoostS*/ 3.0,
                 ...
                 /*ArmingS*/ 1.5},
    /*Seeker*/ SeekerKind::SemiActiveRadar, /*FovHalf*/ 10.0, /*GimbalHalf*/ 50.0};
 ...
 * DLZ interlock (StoresSystem::Release), which reads FireControlBlock — a box a battery does not
 * have. The envelope that decides a surface launch is the SITE's, four numbers per catalogue row
 * (core/Site.h), tested by modules/ground/SiteFireControl before it ever presses the button. */
...
inline constexpr StoreSpec kV750{
    StoreKind::V750, "v750", "v750", 4768.6, 0.9088, 120.0,
    /*Guided*/ true, /*RequiresLock*/ false, /*FuzeRadiusM*/ 12.0, /*WarheadKg*/ 195.0,
    WeaponPerf{/*BoostThrustN*/ 239556.0, /*BoostS*/ 4.5,
                 ...
                 /*ActivationRangeM*/ 0.0, /*SeekerRangeM*/ 0.0,
                 /*ArmingS*/ 3.0},
    /*Seeker*/ SeekerKind::CommandGuided, /*FovHalf*/ 0.0, /*GimbalHalf*/ 0.0, /*GatherS*/ 3.0};
...
inline constexpr StoreSpec kV601{
    StoreKind::V601, "v601", "v601", 2101.0, 0.5112, 90.0,
    /*Guided*/ true, /*RequiresLock*/ false, /*FuzeRadiusM*/ 10.0, /*WarheadKg*/ 70.0,
    WeaponPerf{/*BoostThrustN*/ 179435.0, /*BoostS*/ 2.5,
                 ...
                 /*ActivationRangeM*/ 0.0, /*SeekerRangeM*/ 0.0,
                 /*ArmingS*/ 2.5},
    /*Seeker*/ SeekerKind::CommandGuided, /*FovHalf*/ 0.0, /*GimbalHalf*/ 0.0, /*GatherS*/ 2.5};

/* 3M9 — the 2K12's round, and the ONE surface round outshine already had the weapon side for: the
...
inline constexpr StoreSpec k3m9{
    StoreKind::M3m9, "3m9", "3m9", 1320.6, 0.3959, 90.0,
    /*Guided*/ true, /*RequiresLock*/ false, /*FuzeRadiusM*/ 8.0, /*WarheadKg*/ 59.0,
    WeaponPerf{/*BoostThrustN*/ 51978.0, /*BoostS*/ 4.0,
                 ...
                 /*ActivationRangeM*/ 0.0, /*SeekerRangeM*/ 20000.0,
                 /*ArmingS*/ 2.0},
    /*Seeker*/ SeekerKind::SemiActiveRadar, /*FovHalf*/ 10.0, /*GimbalHalf*/ 45.0, /*GatherS*/ 2.0};
...
inline constexpr StoreSpec k9m33{
    StoreKind::M9m33, "9m33", "9m33", 277.8, 0.1597, 60.0,
    /*Guided*/ true, /*RequiresLock*/ false, /*FuzeRadiusM*/ 6.0, /*WarheadKg*/ 19.0,
    WeaponPerf{/*BoostThrustN*/ 27028.0, /*BoostS*/ 2.0,
                 ...
                 /*ActivationRangeM*/ 0.0, /*SeekerRangeM*/ 0.0,
                 /*ArmingS*/ 1.5},
    /*Seeker*/ SeekerKind::CommandGuided, /*FovHalf*/ 0.0, /*GimbalHalf*/ 0.0, /*GatherS*/ 1.5};

/* 9M32M STRELA-2M — the rear-aspect MANPADS round, SeekerKind::Infrared unchanged. WarheadKg 1.15
...
inline constexpr StoreSpec kStrela2{
    StoreKind::Strela2, "strela2", "strela2", 21.6, 0.0188, 30.0,
    /*Guided*/ true, /*RequiresLock*/ false, /*FuzeRadiusM*/ 2.0, /*WarheadKg*/ 1.15,
    WeaponPerf{/*BoostThrustN*/ 2202.0, /*BoostS*/ 2.0,
                 /*SustainThrustN*/ 0.0, /*SustainS*/ 0.0,
                 ...
                 /*ArmingS*/ 0.6},
    /*Seeker*/ SeekerKind::Infrared, /*FovHalf*/ 0.95, /*GimbalHalf*/ 30.0, /*GatherS*/ 0.6};

/* 9M39 IGLA — the all-aspect MANPADS round. It differs from the Strela in exactly two numbers, its
 * reach and its head's field, and NOT in flare resistance: outshine's flare model is an irradiance
...
inline constexpr StoreSpec kIgla{
    StoreKind::Igla, "igla", "igla", 23.8, 0.0188, 30.0,
    /*Guided*/ true, /*RequiresLock*/ false, /*FuzeRadiusM*/ 2.0, /*WarheadKg*/ 1.17,
    WeaponPerf{/*BoostThrustN*/ 2727.0, /*BoostS*/ 2.0,
                 /*SustainThrustN*/ 0.0, /*SustainS*/ 0.0,
                 ...
                 /*ArmingS*/ 0.6},
    /*Seeker*/ SeekerKind::Infrared, /*FovHalf*/ 2.0, /*GimbalHalf*/ 40.0, /*GatherS*/ 0.6};
 ...
 * [T2, ED p.34-42] is carried instead of the [T4] Mach 2.9 because the rest of the reference base
 ...
 * passive receiver's reach is the EMITTER's gate times the one-way advantage, held by RwrSystem. */
inline constexpr StoreSpec kAgm88{
    StoreKind::Agm88, "agm88", "agm88", 780.0, 0.2345, 120.0,
    /*Guided*/ true, /*RequiresLock*/ false, /*FuzeRadiusM*/ 5.0, /*WarheadKg*/ 66.0,
    WeaponPerf{/*BoostThrustN*/ 15405.0, /*BoostS*/ 6.0,
                 /*SustainThrustN*/ 0.0, /*SustainS*/ 0.0,
                 ...
                 /*ActivationRangeM*/ 0.0, /*SeekerRangeM*/ 0.0,
                 /*ArmingS*/ 1.5},
    /*Seeker*/ SeekerKind::AntiRadiation, /*FovHalf*/ 40.0, /*GimbalHalf*/ 40.0, /*GatherS*/ 0.0,
    /*SeekerMemoryS*/ 4.0};

/* Mk-84, 2 000 lb GP bomb. MassLbs 2 039 nominal and WarheadKg 428.6 (945 lb Tritonal) [T4],
 * corroborated by [T2]. Diameter 18 in, length 151.2 in.
...
inline constexpr StoreSpec kMk84{
    StoreKind::Mk84, "mk84", "mk84", 2039.0, 1.0261, 300.0,
...
inline constexpr StoreSpec kGbu12{
    StoreKind::Gbu12, "gbu12", "gbu12", 610.0, 0.2710, 300.0,
    ...
    WeaponPerf{/*BoostThrustN*/ 0.0, /*BoostS*/ 0.0,
                 ...
                 /*ArmingS*/ 2.0},
    /*Seeker*/ SeekerKind::SemiActiveLaser, /*FovHalf*/ 15.0, /*GimbalHalf*/ 30.0};
 ...
 * carried, and outshine takes the LARGER because it is the more conservative reading of the lethality
...
inline constexpr StoreSpec kCbu87{
    StoreKind::Cbu87, "cbu87", "cbu87", 950.0, 0.7708, 300.0,
    ...
    WeaponPerf{/*BoostThrustN*/ 0.0, /*BoostS*/ 0.0,
                 ...
                 /*ActivationRangeM*/ 0.0, /*SeekerRangeM*/ 0.0,
                 /*ArmingS*/ 2.0},
    /*Seeker*/ SeekerKind::None, /*FovHalf*/ 0.0, /*GimbalHalf*/ 0.0, /*GatherS*/ 0.0,
 ...
 * CARRIAGE refuses a release. Class mass and filling [T3];
 * FAB-500 2 470 x 400 mm, FAB-250 ~1 500 x 285 mm.
 * THE RELEASE ENVELOPE 500-1 000 km/h is [T2, DCS-FM p.75] and the carrier's reference base already rules
 * it "a rejectable condition (out_of_context), not a guideline" — 269.98 / 539.96 kt [DERIVED].
 * CARRIAGE AND BRIEFED RELEASE ONLY: the carrier cannot fly `set task attack` at all (C9 — the real
 * jet's unguided delivery is a DIRECTOR, not a release cue), so these rows exist and their delivery
 * mode does not. */
inline constexpr StoreSpec kFab250{
    StoreKind::Fab250, "fab250", "fab250", 551.2, 0.3987, 300.0,
...
inline constexpr StoreSpec kFab500{
    StoreKind::Fab500, "fab500", "fab500", 1102.3, 0.7854, 300.0,
 ...
 *
 * FOUR OF THE SEVEN ARE SEMI-ACTIVE and therefore BIND THE SHOOTER TO IMPACT (SeekerHandoverS -1).
...
inline constexpr StoreSpec kK13{
    StoreKind::K13, "k13", "k13", 198.4, 0.0586, 60.0,
    /*Guided*/ true, /*RequiresLock*/ true, /*FuzeRadiusM*/ 6.0, /*WarheadKg*/ 7.4,
    WeaponPerf{7912.0, 5.0, 0.0, 0.0, 90.0, 72.8, 0.55, 0.012668, 200.0, 0.0, 3500.0, 1.0},
    /*Seeker*/ SeekerKind::Infrared, /*FovHalf*/ 5.0, /*GimbalHalf*/ 40.0};

/* R-60: the short, light, wide-gimbal round four eastern rows carry. */
inline constexpr StoreSpec kR60{
    StoreKind::R60, "r60", "r60", 97.0, 0.0523, 60.0,
    /*Guided*/ true, /*RequiresLock*/ true, /*FuzeRadiusM*/ 3.8, /*WarheadKg*/ 3.0,
    WeaponPerf{6341.0, 3.0, 0.0, 0.0, 44.0, 35.7, 0.55, 0.011310, 200.0, 0.0, 8000.0, 0.8},
    /*Seeker*/ SeekerKind::Infrared, /*FovHalf*/ 5.0, /*GimbalHalf*/ 45.0};

/* R-24R, "comparable to the AIM-7 Sparrow" [T4] — an eastern BVR round, SARH, bound to impact. */
inline constexpr StoreSpec kR24r{
    StoreKind::R24r, "r24r", "r24r", 489.4, 0.1808, 90.0,
    /*Guided*/ true, /*RequiresLock*/ true, /*FuzeRadiusM*/ 11.0, /*WarheadKg*/ 25.0,
    WeaponPerf{24650.0, 3.0, 9860.0, 5.0, 222.0, 168.5, 0.55, 0.039057, 250.0, 0.0, 35000.0, 1.5},
    /*Seeker*/ SeekerKind::SemiActiveRadar, /*FovHalf*/ 10.0, /*GimbalHalf*/ 50.0};

/* R-40R: the biggest air-to-air round in the tree by a factor of two, carried by an interceptor that
 * cannot turn. WarheadKg 38 is the LOWER bound of the sourced 38-100 kg. */
inline constexpr StoreSpec kR40r{
    StoreKind::R40r, "r40r", "r40r", 1047.2, 0.3493, 120.0,
    /*Guided*/ true, /*RequiresLock*/ true, /*FuzeRadiusM*/ 13.6, /*WarheadKg*/ 38.0,
    WeaponPerf{56226.0, 4.0, 23001.0, 8.0, 475.0, 297.6, 0.55, 0.075477, 300.0, 0.0, 80000.0, 2.0},
    /*Seeker*/ SeekerKind::SemiActiveRadar, /*FovHalf*/ 10.0, /*GimbalHalf*/ 50.0};

/* AIM-7F/M Sparrow. THE PAIRING THIS ROUND EXISTS FOR: a SARH carrier fights a bound fight (cannot
 * defend while supporting) and the same carrier with an active round fights an unbound one. */
inline constexpr StoreSpec kAim7{
    StoreKind::Aim7, "aim7", "aim7", 509.3, 0.1498, 90.0,
    /*Guided*/ true, /*RequiresLock*/ true, /*FuzeRadiusM*/ 14.0, /*WarheadKg*/ 40.0,
    WeaponPerf{35363.0, 3.0, 11788.0, 6.0, 231.0, 154.3, 0.55, 0.032365, 250.0, 0.0, 70000.0, 1.5},
    /*Seeker*/ SeekerKind::SemiActiveRadar, /*FovHalf*/ 10.0, /*GimbalHalf*/ 50.0};

/* Super 530F: a western SARH round. */
inline constexpr StoreSpec kS530f{
    StoreKind::S530f, "s530f", "s530f", 540.1, 0.2514, 60.0,
    /*Guided*/ true, /*RequiresLock*/ true, /*FuzeRadiusM*/ 12.1, /*WarheadKg*/ 30.0,
    WeaponPerf{54837.0, 2.5, 18455.0, 4.0, 245.0, 153.5, 0.55, 0.054325, 250.0, 0.0, 25000.0, 1.5},
    /*Seeker*/ SeekerKind::SemiActiveRadar, /*FovHalf*/ 10.0, /*GimbalHalf*/ 50.0};

/* R550 Magic 1: the widest gimbal in the catalogue, and free at launch like every infrared round. */
inline constexpr StoreSpec kMagic1{
    StoreKind::Magic1, "magic1", "magic1", 196.2, 0.0896, 60.0,
    /*Guided*/ true, /*RequiresLock*/ true, /*FuzeRadiusM*/ 7.9, /*WarheadKg*/ 12.7,
    WeaponPerf{12353.0, 4.0, 0.0, 0.0, 89.0, 67.6, 0.55, 0.019359, 200.0, 0.0, 10000.0, 1.0},
    /*Seeker*/ SeekerKind::Infrared, /*FovHalf*/ 5.0, /*GimbalHalf*/ 55.0};

/* ---- THE STORE THAT IS NOT A WEAPON: a 370-gallon wing tank ---------------------------------------
...
inline constexpr StoreSpec kTank370{
    StoreKind::Tank370, "tank370", "tank370", 240.0, 0.475, 120.0,
    /*Guided*/ false, /*RequiresLock*/ false, /*FuzeRadiusM*/ 0.0, /*WarheadKg*/ 0.0,
    WeaponPerf{/*BoostThrustN*/ 0.0, /*BoostS*/ 0.0,
                 /*SustainThrustN*/ 0.0, /*SustainS*/ 0.0,
...
inline constexpr const StoreSpec *kStoreCatalogue[] = {&kMk82, &kAim120, &kAim9, &kR73, &kR27r,
...
inline const StoreSpec *FindStore(const char *key) {
  if (!key) return nullptr;
  for (const StoreSpec *s : kStoreCatalogue)
    if (std::strcmp(s->Key, key) == 0) return s;
  return nullptr;
}

inline const StoreSpec *StoreSpecOf(StoreKind kind) {
  for (const StoreSpec *s : kStoreCatalogue)
 ...
 * pilot does not pick a release moment, because the AIRCRAFT does. */
enum class DeliveryMode : uint8_t { Ccip = 0, Ccrp, Arm, Opt };

inline const char *DeliveryModeStr(DeliveryMode m) {
  switch (m) {
    case DeliveryMode::Ccrp: return "ccrp";
    case DeliveryMode::Arm: return "arm";
    case DeliveryMode::Opt: return "opt";
    case DeliveryMode::Ccip: break;
  }
  return "ccip";
}
...
struct ReleaseSolution {
  bool   Valid = false;
  DeliveryMode Mode = DeliveryMode::Ccip;
...
struct StoreRelease {
  ...
  ArTargetClass ArClass = ArTargetClass::AnySurface;
};
