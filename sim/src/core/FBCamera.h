...
/* Split out of FBCameraBasisFrom so the ECEF path reuses the EXACT same silent-mirror-critical logic
 * instead of a second copy that could drift from it. */
inline void FBCameraAxes(float yawDeg, float pitchDeg, float rollDeg, float f[3], float sr[3],
                         float up[3]) {
   ...
   * screen-right. FBVec3Normalize leaves a sub-epsilon vector alone, so the frame degrades to
...
inline FBCameraBasis FBCameraBasisFrom(float yawDeg, float pitchDeg, float rollDeg, const float eye[3],
                                       float fovDeg, float aspect, float znear, float zfar) {
  const float RAD = (float)M_PI / 180.f;
  FBCameraBasis c;
  FBCameraAxes(yawDeg, pitchDeg, rollDeg, c.f, c.sr, c.up);

  float ctr[3] = {eye[0] + c.f[0], eye[1] + c.f[1], eye[2] + c.f[2]};
  float view[16], proj[16];
  FBMat4LookAt(view, eye, ctr, c.up);
  FBMat4Perspective(proj, fovDeg * RAD, aspect, znear, zfar);
  FBMat4Mul(c.mvp, proj, view);
  return c;
}

/* FBCameraBasisFrom above is NOT dead: sky dome and star field are an infinity pass in LOCAL
 * render-ENU, so they keep the flat-earth camera — the horizon dip emerges because the ECEF terrain
 * curves away below that local horizon. The rotation into ECEF axes uses core/FBGeodesy.h's
 * FBEnuAxesEcef (closed form, right-handed, det +1, so triangle winding survives) and is the OTHER
...
inline void FBMvpTranslate(float out[16], const float vp[16], const float t[3]) {
  for (int i = 0; i < 12; i++)
    out[i] = vp[i];
  for (int r = 0; r < 4; r++)
...
inline FBFrustum FBFrustumFrom(const float mvp[16]) {
  FBFrustum fr;
 ...
 * would be a hole. */
inline int FBAabbVisible(const FBFrustum *fr, const float bmin[3], const float bmax[3]) {
  for (int k = 0; k < 6; k++) {
