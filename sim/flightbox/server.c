...
static void msp_poll(void){
    ...
    while(i+3<msp_rxn){
        if(msp_rx[i]=='$'&&msp_rx[i+1]=='M'&&msp_rx[i+2]=='>'){
            ...
            else if(cmd==121&&ln>=1){ int ns=pl[1];   /* MSP_NAV_STATUS nav-state (iNav MW_NAV_STATE_*) */
                /* Map iNav's REAL nav state to the annunciator (MIL-STD-1787: show the actual active mode).
                   RTH family (start/enroute/hover-home/rth-climb) -> RTH; POSHOLD (hold infinit/timed) ->
                   LOITER; flying a mission (WP enroute/process-next/jump/autoland) -> AUTO. Debounce a
                   transient nav_state==0 between polls (used to strobe the annunciator ~6x/s): hold the last
                   confirmed mode until 0 PERSISTS (genuinely manual/no-nav). */
                static int zero_run=0;
                if(ns==0){ if(++zero_run>=6) mt_state = mt_arm?ST_MANUAL:ST_ARMED; }
                else { zero_run=0;
                    mt_state = (ns==1||ns==2||ns==13||ns==15) ? ST_RTH
                             : (ns==3||ns==4)                 ? ST_LOITER
                             :                                  ST_AUTO; } }
            i+=6+ln;
