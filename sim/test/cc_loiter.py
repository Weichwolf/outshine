...
def ring():
    """N points on the circle of RADIUS around (LAT,LON), plus a JUMP-to-1 waypoint (infinite loiter)."""
    wps = []
    for i in range(N):
        a = 2*math.pi*i/N
    ...
    wps.append((0.0, 0.0, 0.0, 6, 1, -1))                        # action 6 = JUMP -> WP1, repeat -1 = forever
    return wps
...
def send_wp(m, i, w):
    ...
    b[14:16]= struct.pack("<h", p1 & 0xffff if p1 >= 0 else p1)  # p1 (JUMP target, 1-based)
    b[16:18]= struct.pack("<h", p2)                              # p2 (JUMP repeats, -1 = infinite)
    b[20]   = 0xa5 if i == N else 0                              # last WP flag on the JUMP entry
    m.send(209, bytes(b))

def main():
    print(f"[loiter] {AC} circling {RADIUS:.0f} m @ {ALT:.0f} m over {LAT:.4f},{LON:.4f} ({N} WPs + JUMP)")
    spawn()
    def connect():
        for _ in range(400):
            try:
    ...
    while time.monotonic()-t0 < SECS:
        ...
        try:
            ...
            m = connect(); up = False; continue                        # SITL dropped -> reconnect, re-upload WPs
        af = struct.unpack("<I", st[9:13])[0] if st and len(st) >= 13 else 0
        armed = bool(af & 0x4)
