...
def mission_text(name, timeout, geometry, west_var, east_var):
    ...
    west = UNIT_TPL[west_var.module].format(
        call="west", team="friendly", lat=wlat, lon=wlon, alt=walt, hdg="90.0",
        kt=wkt, wplat=wlat, wplon=wlon + 1.95, foe="east",
        tuning="".join("  set %s %g\n" % kv for kv in sorted(west_var.params.items())))
    east = UNIT_TPL[east_var.module].format(
        call="east", team="hostile", lat=elat, lon=elon, alt=ealt, hdg="270.0",
...
class Variant:
    def __init__(self, name, params, module="f16"):
        self.name = name
        self.params = params
        self.module = module
...
def load_variants(path):
    ...
    for lineno, raw in enumerate(open(path), 1):
        ...
        out.append(Variant(tok[0], params, module))
    if len(out) < 2:
