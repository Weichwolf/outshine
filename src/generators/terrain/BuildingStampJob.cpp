...
BuildingStampJob::Advance(std::span<const Ground::BuildingField::Footprint> footprints,
                          std::span<const double> points,
                          uint64_t vectorGeneration,
                          size_t workMost) {
  if (vectorGeneration != VectorGeneration_) {
    return std::unexpected("building stamps require their pinned vector generation");
  }
  if (workMost == 0) { return std::unexpected("building stamp work budget is zero"); }
  if (!FootprintCount_) {
  ...
  size_t visited = 0;
  while (visited < workMost && Phase_ != Phase::Done) {
...
size_t BuildingStampJob::HeapBytes() const noexcept {
  ...
  if (Current_) { held += Current_->HeapBytes(); }
  return held;
}
