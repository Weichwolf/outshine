...
fn shade(n : vec3f, p : vec3f, albedo : vec3f) -> vec3f {
  let v = normalize(-p);
  let a = surface.roughness * surface.roughness;
  let a2 = a * a;
  let diffuseColour = albedo * (1.0 - surface.metalness);
  let f0 = mix(vec3f(kDielectricF0), albedo, surface.metalness);
...
void SubjectDraw::Configure(const Gpu &gpu) {
  ...
  /* TWENTY-FOUR PIPELINES: four vertex layouts, two facings, three alpha modes. The facing is the SLOT's
  ...
  for (const SurfaceKind kind : {SurfaceKind::Opaque, SurfaceKind::Masked, SurfaceKind::Blended}) {
    ...
    for (const VertexLayout layout : {VertexLayout::Position, VertexLayout::PositionUv,
      ...
      rp.vertex.buffers = lit ? (textured ? litTexturedLayouts : litLayouts)
...
void SubjectDraw::BindSurface(const SubjectMaterial &material) {
  ...
  const float surface[kSurfaceFloats] = {
      ...
      row.Emission[0],     row.Emission[1],   row.Emission[2],   0.0f};
