/* The WGSL half of core/CloudDensity.h — the SAME formula, transliterated. Two rules keep the two
 * halves from drifting: (a) the numeric constants are not typed here at all, they are emitted from the
 * C++ ones by CloudDensityConstsWGSL(); (b) `gpu_native --cloudcheck` evaluates both over a sample set
 * and prints the largest difference. A textual splice, like AtmoCommon.h — never compiled alone. */
#ifndef CLOUDDENSITYWGSL_H
#define CLOUDDENSITYWGSL_H
...
/* Mirrors CloudDeckParams field for field, in declaration order — the uniform/storage upload is a
...
/* ONE source of truth for the numbers: printed from core/CloudDensity.h with enough digits to be
 * exact for a float. Prepended ahead of kCloudDensityWGSL by every consumer. */
inline std::string CloudDensityConstsWGSL(void) {
  char buf[1024];
