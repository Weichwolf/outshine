...
bool Camera::Projection(double viewportAspect, Transform &out) const {
  ...
  if (Kind == CameraKind::Perspective) {
    if (!(viewportAspect > 0) || !std::isfinite(viewportAspect) || !(YfovRad > 0) ||
        YfovRad >= std::numbers::pi || !(ZNearM > 0) || !std::isfinite(ZNearM) ||
        std::isnan(ZFarM) || ZFarM < 0) {
      return false;
    }
  ...
  if (!(XMagM > 0) || !std::isfinite(XMagM) || !(YMagM > 0) || !std::isfinite(YMagM) ||
      !std::isfinite(ZNearM) || !std::isfinite(ZFarM) || !(ZFarM > ZNearM)) {
    return false;
  }
  out.M[0] = 1.0 / XMagM;
  out.M[5] = 1.0 / YMagM;
